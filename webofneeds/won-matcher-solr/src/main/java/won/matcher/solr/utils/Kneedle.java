 ...
 *
 * NOTE: This implementation does not check the concavity of the curve. Also no smoothing of the curve is applied by
 * this method.
 *
 * Kneedle algorithm described in: Satopaa, V., Albrecht, J., Irwin, D., & Raghavan, B. (2011, June). Finding a"
 * Kneedle" in a Haystack: Detecting Knee Points in System Behavior. In 2011 31st International Conference on
...
public class Kneedle {
    ...
    /**
     * Detect all knee points in a curve according to "Kneedle" algorithm. Alternatively this method can detect elbow
     * points instead of knee points.
     *
     * @param x
     *            x-coordintes of curve, must be increasing in value
     * @param y
     *            y-coordinates of curve
     * @param detectElbows
     *            if true detects elbow points, if false detects knee points
    ...
    private int[] detectKneeOrElbowPoints(final double[] x, final double[] y, boolean detectElbows) {
        ...
        for (int i = 1; i < y.length - 1; i++) {
            ...
            if (yDiff[i] > yDiff[i - 1] && yDiff[i] > yDiff[i + 1]) {
                ...
                // NOTE: As stated in the paper the threshold Tlmx is computed. Since the mean distance of all
                // consecutive
                // x-values summed together for a normalized function is always (1 / (n -1)) we do not have to compute
                // the
