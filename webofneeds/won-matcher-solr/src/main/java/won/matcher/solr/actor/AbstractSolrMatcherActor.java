...
public abstract class AbstractSolrMatcherActor extends UntypedActor
{
  ...
  protected void processNeedEvent(NeedEvent needEvent)
    ...
    if (needModelWrapper.hasFlag(WON.SUPPRESS_HINTS_FOR_ME) && needModelWrapper.hasFlag(WON.SUPPRESS_HINTS_FOR_COUNTERPART)) {
      log.info("Discarding received need due to flags won:SuppressHintsForMe and won:SuppressHintsForCounterpart: {}", needEvent);
      return;
    }
