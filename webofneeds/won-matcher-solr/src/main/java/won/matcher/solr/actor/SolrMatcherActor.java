...
public class SolrMatcherActor extends UntypedActor {
    ...
    protected void processActiveNeedEvent(NeedEvent needEvent) throws IOException, SolrServerException, JsonLdError {
        ...
        if (!needModelWrapper.hasFlag(WON.NO_HINT_FOR_COUNTERPART)) {
            // execute the query
            log.info("query Solr endpoint {} for need {} and need list 2 (without NoHintForSelf, excluding WhatsAround needs)",
                            config.getSolrEndpointUri(usedForTesting), needEvent.getUri());
            ...
            if (docs != null) {
                ...
                log.warning("No results found for need list 2 (without NoHintForSelf, excluding WhatsAround needs) query of need ",
                                needEvent);
            }
        }
        ...
        if (!needModelWrapper.hasFlag(WON.NO_HINT_FOR_COUNTERPART)) {
            ...
            // execute the query
            log.info("query Solr endpoint {} for need {} and need list 3 (without NoHintForSelf that are only WhatsAround needs)",
                            config.getSolrEndpointUri(usedForTesting), needEvent.getUri());
            SolrDocumentList docs = queryExecutor.executeNeedQuery(queryString, config.getMaxHintsForCounterparts(),
                            null, filterQueries.toArray(new String[filterQueries.size()]));
            if (docs != null) {
                ...
                log.warning("No results found for need list 3 (without NoHintForSelf that are only WhatsAround needs) query of need ",
                                needEvent);
            }
        }
        // index need
