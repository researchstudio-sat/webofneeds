...
(function() {
  ...
  /*async*/ function loadStarshapedGraph(store, startUri, tree) {
    ...
    const tmpResultP = new Promise((resolve, reject) =>
      store.graph(
        (success, result) =>
          success ? resolve(result) : reject("Couldn't get graph ")
      )
    );
    const resultGraphP = tmpResultP.then(tmpResult => {
      try {
        ...
        if (!resultGraph) {
          throw new Error(
            "Failed to construct valid graph:" +
              resultGraph +
              ". For start Uri " +
              startUri +
              " and tree " +
  ...
  function loadStarshapedGraph_internal(store, dataGraph, startNode, tree) {
    ...
    let pathResult = loadGraphForPath(
      store,
      dropUnnecessaryTriples(store, dataGraph, path),
      startNode,
      path
    );
