...
import "style/_instrumentpicker.scss";

const serviceDependencies = ["$scope", "$element"];
function genComponentConf() {
  let template = `
      <div ng-repeat="dtl in self.personDetails" class="ip__detail">
          <div class="ip__detail__label">
              {{dtl.fieldname}}
          </div>
          <input
            type="text"
            id="{{dtl.name}}"
            class="ip__detail__input"
            ng-model="dtl.value"
            won-input="::self.updateDetails(dtl)"
            ng-class="{'ip__detail__input--withreset' : self.showResetButton(dtl.name)}"
          />
          <svg class="ip__detail__icon clickable" 
  ...
  class Controller {
    constructor() {
      ...
      this.personDetails = [
        ...
        { fieldname: "Instrument", name: "instrument", value: undefined },
        { fieldname: "Genre", name: "genre", value: undefined },
      ];
...
export default angular
  .module("won.owner.components.instrumentPicker", [wonInput])
  .directive("wonInstrumentPicker", genComponentConf).name;
