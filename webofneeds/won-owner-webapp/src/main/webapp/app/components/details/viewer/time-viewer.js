...
import "style/_time-viewer.scss";

const serviceDependencies = ["$scope", "$element"];
function genComponentConf() {
  let template = `
        <div class="timev__header">
          <svg class="timev__header__icon" ng-if="self.detail.icon">
              <use xlink:href={{self.detail.icon}} href={{self.detail.icon}}></use>
          </svg>
          <span class="timev__header__label" ng-if="self.detail.label">{{self.detail.label}}</span>
        </div>
        <div class="timev__content">{{ self.content }}</div>
    `;

  class Controller {
    constructor() {
      attach(this, serviceDependencies, arguments);
      window.timev4dbg = this;
...
export default angular
  .module("won.owner.components.timeViewer", [])
  .directive("wonTimeViewer", genComponentConf).name;
