...
public class WonWebSocketHandler extends TextWebSocketHandler implements WonMessageProcessor, InitializingBean {
     ...
     * FacetType.ChatFacet.getURI().toString()}); NeedPojo createdNeedPojo = resolve(needPojo); Need need =
     * needRepository.findOne(createdNeedPojo.getNeedId()); user.getNeeds().add(need); wonUserDetailService.save(user);
     * HttpHeaders headers = new HttpHeaders(); headers.setLocation(need.getNeedURI()); return new
     * ResponseEntity<NeedPojo>(createdNeedPojo, headers, HttpStatus.CREATED);
     */
    @Override
        ...
        try {
            ...
            if (wonMessage.getMessageType() == WonMessageType.DELETE) {
                ...
                try {
                    ...
                    /*
                     * //Get the user from owner application db user = userRepository.findOne(user.getId()); //Delete
                     * need in users need list and save changes user.deleteNeedUri(userNeed); userRepository.save(user);
    ...
    /*
     * update the session last accessed time, - spring-session was added to synchronize // http sessions with websocket
     * session // see: http://spring.io/blog/2014/09/16/preview-spring-security-websocket-support- sessions // Currently
     * used here Sping's MapSession implementation of Spring has default timeout of 30 minutes
     */
    ...
    /**
     * We want to keep the buffer in the underlying server small (8k per websocket), but still be able to receive large
     * messages. Hence, we have to be able to handle partial messages here.
     */
    @Override
    ...
    public WonMessage process(final WonMessage wonMessage) {
        ...
        try {
            ...
            if (webSocketSessions.size() == 0) {
                logger.debug(
                        "cannot deliver message of type {} for need {}, receiver {}: no websocket session found. Trying to send message by email.",
    ...
    private void notifyPerEmail(final User user, final URI needUri, final WonMessage wonMessage) {
        ...
        if (wonMessage.getEnvelopeType() == WonMessageDirection.FROM_OWNER) {
            // we assume that this message, coming from the server here, can only be an echoed message. don't send by
            // email.
        ...
        if (wonMessage.getMessageType() == WonMessageType.SUCCESS_RESPONSE) {
            if (WonMessageType.CREATE_NEED == wonMessage.getIsResponseToMessageType()) {
                ...
                try {
                    ...
                    logger.warn("could not associate need {} with currently logged in user, cannot send message",
                            needUri, e);
                    if (user != null) {
                        webSocketSessionService.removeMapping(user, session);
                    }
