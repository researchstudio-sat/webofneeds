...
public class ServerSideActionService implements WonMessageProcessor {
    ...
    public void connect(List<FacetToConnect> facets, Authentication authentication) {
        ...
        final Function<Optional<WonMessage>, Collection<EventTriggeredAction<WonMessage>>> action =
            new Function<Optional<WonMessage>, Collection<EventTriggeredAction<WonMessage>>>() {
                @Override
                public Collection<EventTriggeredAction<WonMessage>> apply(Optional<WonMessage> msg) {
                    //process success responses for create
                    if (msg.isPresent() 
                            && needCounter.get() < 2 
                            && isResponseToCreateOfFacets(msg.get(), facets)
        ...
        this.actionContainer.addAction(new EventTriggeredAction<WonMessage>(String.format("Connect %s and %s", fromFacet, toFacet), action));
    ...
    private boolean isResponseToCreateOfFacets(WonMessage msg, List<FacetToConnect> facets) {
        return facets
                ...
                .anyMatch( facet -> 
                        facet.isPending() 
                        && isResponseToCreateOfFacet(msg, facet));    
    }
    ...
    private void sendConnect(URI fromFacet, URI toFacet, Authentication authentication){
        ...
        WonMessage msgToSend =  WonMessageBuilder.setMessagePropertiesForConnect(
                ...
                Optional.of(toFacet), toNeedURI, toWonNodeURI, "Connect message automatically sent by a server-side action")
                .build();
        try {
    ...
    private void sendOpen(WonMessage connectMessageToReactTo, Authentication authentication) {
        ...
        URI messageURI = wonNodeInformationService.generateEventURI(fromWonNodeURI);
        WonMessage msgToSend = WonMessageBuilder.setMessagePropertiesForOpen(
                messageURI, connectMessageToReactTo, 
                "Open message automatically sent by a server-side action")
                .build();
        try {
            AuthenticationThreadLocal.setAuthentication(authentication);
            ownerApplicationService.sendWonMessage(msgToSend);
