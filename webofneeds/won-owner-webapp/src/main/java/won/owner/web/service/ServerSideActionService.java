...
@Component public class ServerSideActionService implements WonMessageProcessor {
  ...
  private boolean isConnectFromFacetForFacet(WonMessage msg, URI sender, URI receiver) {
    return msg.getMessageType() == WonMessageType.CONNECT && msg.getEnvelopeType() == WonMessageDirection.FROM_EXTERNAL
  ...
  private void sendConnect(URI fromFacet, URI toFacet, Authentication authentication) {
    ...
    WonMessage msgToSend = WonMessageBuilder
        .setMessagePropertiesForConnect(messageURI, Optional.of(fromFacet), fromNeedURI, fromWonNodeURI,
