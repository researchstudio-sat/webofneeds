...
public class BridgeForLinkedDataController implements InitializingBean {
    ...
    /*
     * //for some reason this cannot be used as parameter in restTemplate.execute() private final ResponseExtractor
     * httpResponseResponseExtractor = new ResponseExtractor<ClientHttpResponse>() {
     * 
     * @Override public ClientHttpResponse extractData(final ClientHttpResponse response) throws IOException { return
     * response; } };
     */
    ...
    /**
     * Currently copies all the headers from the given headers to the headers of the response. TODO check with spec if
     * any other headers should not be copied (e.g , it seems Transfer-Encoding should not be copied by proxies, see
    ...
    /**
     * Check if the current user has the claimed identity represented by web-id of the need. I.e. if the identity is
     * that of the need that belongs to the user - return true, otherwise - false.
     *
