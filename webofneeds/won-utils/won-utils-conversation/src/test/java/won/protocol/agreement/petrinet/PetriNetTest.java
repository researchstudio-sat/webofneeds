...
public class PetriNetTest {
    ...
    public void testLoadFromString() {
        ...
        Assert.assertTrue(net.getPlaces().stream().anyMatch(place -> place.getId().equals("http://purl.org/webofneeds/process/taxi#PassengerReady")));
    }
    ...
    public void testEnabledTransitions()  {
        ...
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PassengerArrivedAtPickupLocation")));
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#DriverArrivedAtPickupLocation")));
    }
    ...
    public void testSuccessfulTransation()  {
        ...
        animator.fireTransition(transitions.stream().filter(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PassengerArrivedAtPickupLocation")).findFirst().get());
        transitions = animator.getEnabledTransitions();
        //expected enabled transitions:
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PassengerStoppedWaiting")));
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#DriverArrivedAtPickupLocation")));
        Assert.assertEquals(2, transitions.size());
        //actual transition: driver arrives
        animator.fireTransition(transitions.stream().filter(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#DriverArrivedAtPickupLocation")).findFirst().get());
        transitions = animator.getEnabledTransitions();
        //expected enabled transitions:
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#ArrivedAtDestination")));
        Assert.assertEquals(1, transitions.size());
        //actual transition: arrive at destination
        animator.fireTransition(transitions.stream().filter(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#ArrivedAtDestination")).findFirst().get());
        transitions = animator.getEnabledTransitions();
        //expected enabled transitions:
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PaymentMade")));
        Assert.assertEquals(1, transitions.size());
        //actual transition: payment is made
        animator.fireTransition(transitions.stream().filter(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PaymentMade")).findFirst().get());
        transitions = animator.getEnabledTransitions();
        //expected enabled transitions:
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#PassengerReviewedDriver")));
        Assert.assertTrue(transitions.stream().anyMatch(t -> t.getId().equals("http://purl.org/webofneeds/process/taxi#DriverReviewedPassenger")));
        Assert.assertEquals(2, transitions.size());
    }
