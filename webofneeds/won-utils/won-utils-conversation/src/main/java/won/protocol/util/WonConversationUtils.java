...
public class WonConversationUtils {
  ...
  public static AgreementProtocolState getAgreementProtocolState(URI connectionUri, LinkedDataSource linkedDataSource) {
    ...
    while (true) {
      ...
      try {
        Dataset conversationDataset = WonLinkedDataUtils
        ...
        if (!recrawl(recrawledForIncompleteness, connectionUri, needUri, linkedDataSource, e.getMissingMessageUri(),
            e.getReferringMessageUri())) {
          throw e;
        }
  ...
  private static void refreshDataForConnection(URI connectionUri, URI needUri, LinkedDataSource linkedDataSource) {
    ...
    URI connectionEventContainerUri = WonLinkedDataUtils
        .getEventContainerURIforConnectionURI(connectionUri, linkedDataSource);
    invalidate(connectionEventContainerUri, needUri, linkedDataSource);
    URI remoteConnectionUri = WonLinkedDataUtils
        .getRemoteConnectionURIforConnectionURI(connectionUri, linkedDataSource);
  ...
  private static boolean recrawl(Set<URI> recrawled, URI connectionUri, URI needUri, LinkedDataSource linkedDataSource,
      URI... uris) {
