...
public class DeliveryChain {
    ...
    public void addMessage(ConversationMessage msg) {
        if (msg.isHeadOfDeliveryChain()) {
            if (this.head != null && this.head != msg) {
                throw new IllegalArgumentException("Trying to add another head (" + msg.getMessageURI()
                        + ") to delivery chain " + head.getMessageURI());
            }
            this.head = msg;
        }
    ...
    public boolean isTerminated() {
        ...
        default:
            ...
            return head.hasResponse() && head.hasCorrespondingRemoteMessage()
                    && head.getCorrespondingRemoteMessageRef().hasResponse() && head.getCorrespondingRemoteMessageRef()
                            .getIsResponseToInverseRef().hasCorrespondingRemoteMessage();
        }
    }
    ...
    /**
     * Another delivery chain is interleaved with this one both head messages are before either chain.
     * 
     * @param other
     * @return
     */
