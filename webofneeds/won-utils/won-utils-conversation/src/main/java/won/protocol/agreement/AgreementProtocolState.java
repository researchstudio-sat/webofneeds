...
public class AgreementProtocolState {
    ...
    /**
     * Returns the set of effects (<code>MessageEffect</code>s) of the head of the delivery chain that the specified
     * message belongs to, or an empty set if no message is found for that URI.
     * 
     * @param messageUri
     * @return
     */
    ...
    /**
     * Returns a list of all agreement URIs in the order they were made. The oldestFirst parameter controls ascending or
     * descending order.
     */
    ...
    private void logNthLatestMessage(int n, URI needUri, String type, URI result) {
        logger.debug(n + "-th latest message " + (type == null ? "" : "of type " + type)
                + (needUri == null ? "" : " sent by " + needUri) + ": " + (result == null ? " none found" : needUri));
    }
    ...
    public URI getLatestPendingProposesMessageSentByNeed(URI needUri) {
        URI uri = getNthLatestMessage(m -> needUri.equals(m.getSenderNeedURI()) && m.isProposesMessage()
                && m.getEffects().stream().anyMatch(e -> e.isProposes() && isPendingProposal(m.getMessageURI())), 0);
        if (logger.isDebugEnabled()) {
            logNthLatestMessage(0, needUri, null, uri);
        }
        return uri;
    ...
    public URI getLatestProposesToCancelMessageSentByNeed(URI needUri) {
        URI uri = getNthLatestMessage(
                m -> needUri.equals(m.getSenderNeedURI()) && m.isProposesToCancelMessage()
                        && m.getEffects().stream().anyMatch(e -> e.isProposes() && e.asProposes().hasCancellations()),
                0);
        if (logger.isDebugEnabled()) {
            logNthLatestMessage(0, needUri, null, uri);
        }
    ...
    public URI getLatestPendingProposal(Optional<ProposalType> type, Optional<URI> senderNeedUri) {
        URI uri = getNthLatestMessage(m -> (m.isProposesMessage() || m.isProposesToCancelMessage())
                ...
                && m.getEffects().stream().filter(
                        e -> e.isProposes() && (!type.isPresent() || e.asProposes().getProposalType() == type.get()))
    ...
    public URI getLatestPendingProposalOrClaim(Optional<ProposalType> type, Optional<URI> senderNeedUri) {
        URI uri = getNthLatestMessage(m -> (m.isProposesMessage() || m.isProposesToCancelMessage()
                ...
                && m.getEffects().stream().filter(
                        e -> e.isProposes() && (!type.isPresent() || e.asProposes().getProposalType() == type.get())
                                || e.isClaims())
                        .map(e -> e.getMessageUri()).anyMatch(msgUri -> isPendingProposal(msgUri)
                                || isPendingCancellation(msgUri) || isClaim(msgUri)),
                0);
        if (logger.isDebugEnabled()) {
            logNthLatestMessage(0, senderNeedUri.orElse(null), null, uri);
        }
    ...
    public Optional<String> getTextMessage(URI messageUri) {
        ...
        return head.getContentGraphs().stream()
                .flatMap(contentGraphURI -> WonRdfUtils.MessageUtils
            ...
            if (msg.isAcceptsMessage()) {
                ...
                msg.getAcceptsRefs().stream().filter(other -> msg != other)
                        ...
                        .filter(other -> msg.isMessageOnPathToRoot(other)).filter(other -> {
                            ...
                            if (changedSomething) {
                                effectsBuilder.accepts(other.getMessageURI(),
                                        other.getProposesToCancel().stream().collect(Collectors.toSet()));
                            }
                        });
            }
    ...
    private Dataset acknowledgedSelection(Dataset conversationDataset, Collection<ConversationMessage> messages) {
        ...
        messages.stream().forEach(message -> {
            ...
            if (msgChain.containsOtherChains()) {
                ...
                // one has been delayed maliciously. Removing it is the only safe option. Downside: It allows the
                // recipient of the
                // message to delay it such that it is removed by this rule, but that at least has immediate effects:
                // the
                ...
                msgChain.getInterleavedDeliveryChains().stream().filter(otherChain -> otherChain.isTerminated())
                        .forEach(otherChain -> {
                            // "interleaved" relationship is symmetric -> drop this message (chain), the other message
                            // will be dropped when it is processed
                            if (logger.isDebugEnabled()) {
    ...
    private boolean removeCancellationProposal(URI proposalUri) {
        ...
        StmtIterator it = cancellationProposals.listStatements(
                cancellationProposals.getResource(proposalUri.toString()), WONAGR.PROPOSES_TO_CANCEL, (RDFNode) null);
