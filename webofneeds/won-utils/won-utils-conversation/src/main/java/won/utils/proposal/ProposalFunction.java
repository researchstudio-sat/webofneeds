...
public class ProposalFunction {
    ...
    public Dataset applyProposalFunction(Dataset conversationDataset){
        ...
        try (QueryExecution queryExecution = QueryExecutionFactory.create(query, conversationDataset)) {
            ...
            //add the last model
            if (currentProposal != null) {
            	result.addNamedModel(currentProposal.asResource().getURI()+PROPOSAL_SUFFIX, currentProposalContent);
            }
            return result;
        } finally {
