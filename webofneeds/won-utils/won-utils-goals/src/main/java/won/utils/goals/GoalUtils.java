...
/**
 * Utils class for handling goals related actions like graph blending and data
 * extraction of goals instantiation.
 */
public class GoalUtils {
  ...
  /**
   * Extracts data from a data model while considering shacl shapes expressing
   * constraints on that data model. The method executes shacl validation using
   * the shapes model on the data model. The goal data is extracted from the
   * resulting report as well as the shapes and data model. A specific sparql
   * query loaded from resources specifies which data will be extracted. The
   * extracted data will contain all focus nodes that were actually referenced in
   * the shapes model and only these properties that were referenced in the shapes
   * model. For details on the extraction refer to the sparql queries:
   * /won/utils/goals/extraction/goal-extraction-only-referenced-properties.rq
   *
   * @param dataModel        The data model is expected to contain the rdf data
   *                         which is usually the merged data of two needs and
   *                         their conversation from which the goal data is gonna
   *                         be extracted.
