...
*	Authentication of the Need or Node accessing a linked data resource with restricted access
...
resource linked data and extracting it from there. Then this public key is used to verify the authenticity of the data
 generated by Need or Node.

Authentication of a WoN entity (Need or Node) when accessing a WoN linked data resource with 
...
Node and Owner can serve as both client and server. Matcher, currently, only serves as client in WoN. Needs are also 
clients, and in the current architecture, since their key management is done in the Owner Application, are represented
by the Owner acting as Node's client on behalf of those Needs. 

For example, when WoN Linked Data is being requested from the Node, or when an Owner application sends a WoN message to
the Node on behalf of its user's Need for publishing and processing, the Node serves as a server and the Owner as 
...
the user (from browser) sends to the Owner application a WoN message related to his Need.
...
the case for Need and Node, and in WoN is applied during WebID-TLS verification when accessing Linked Data 
...
connection. In WoN the Owner, Node, Matcher and Needs have their certificate. 
...
The Needs certificates are provided 
when a Message stored as a Linked Data on a Node is being accessed by that Need (Owner application accesses it on 
behalf of Need and provides that Need's certificate, of course, only if the current user is eligible owner of that 
Need) - on [WoN HTTPS channel](data-channels.md).
...
i.e. those of Needs, Owner client, Node client and Matcher client.
...
for Needs and Owner, in [owner properties](../../conf/owner.properties) in

* keystore.password - password to access the keystore
* keystore.location - location of the keystore, if not found there - generated and saved there
* uri.prefix.resource - used as alias of the owner's key and certificate, the aliases of the needs are their URIs
