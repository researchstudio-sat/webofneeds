 ...
 * The service provider remains the central point which releases blind signatures and manages the ratings.
 *
 * After the transaction of a payment is successful, a rating-process is initiated.
 * Bob and Alice generate a random hash via SHA512 and exchange it - test_REP_rdf_message_exchangeRandomHash()
 * Bob receives the hash from Alice and signs it with his private key.
 * Bob forms a reputation token, which contains his certificate and the signed hash.
 * The certificate contains the public key which matches the private key with which the hash was signed.
 * Both Bob and Alice perform the following procedure:
 *
 * form a reputation token, which contains its certificate and the signed hash.
 * The certificate contains the public key that matches the private key with which the hash was signed.
 * Send Reputation Token to Service Provider, test_REP_rdf_message_blindSign()
 *
 * Service Provider creates a blind signature of the token and sends it back, test_REP_rdf_message_blindSignFinish()
 *
 * Replacing the Reputation Token with the blindly signed token, test_REP_rdf_message_exchangeReputationToken()
 *
 *
 * As soon as Bob receives the reputation token together with the blindly signed reputation token,
 * the signature is verified using the public key.
 * Bob then sends the reputation token, blindly signed token and its original hash to the service provider
 *    - test_REP_rdf_message_verifyReputationToken()
 *    - which checks whether the blind signature is correct and whether the random hash can be verified with the signed hash.
 *    The status is communicated with test_REP_rdf_message_verificationState().
 *
 * If the response from the service provider contains 'true', Bob sends his rating to the service provider -
...
public class WonMessageRepTest {
    ...
    public void test_REP_rdf_message_blindSignFinish() {
        ...
        baseRes.addProperty(REP.REPUTATIONTOKEN, reputationToken);
        baseRes.addProperty(REP.BLIND_SIGNED_REPUTATIONTOKEN, "jKrRkeUktfOYVxj1UWIKBDN+5aICIBNAkNBtG3TelP+LOK9sMHNy2YApjtv/nw4GwEq6tTsYZMMmptah+8nc9m3siM6HuxspbI/gT6ZcveUQSKvBbDQk00GVrCgpyA8rYmE4QFPEjLYGnXF6/QpUP9nt/dR1kX54YBWBTTgYDBU=");
