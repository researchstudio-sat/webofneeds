...
public class CryptographyUtils {
  ...
  private static SSLContext getSSLContext(final KeyStore keyStore, final String ksPass,
      final PrivateKeyStrategy keyStrategy, final KeyStore trustStore, TrustStrategy trustStrategy, boolean allowCached)
      throws Exception {
    if (allowCached && keyStrategy instanceof PredefinedAliasPrivateKeyStrategy) {
      return getCachedSslContextForPredefinedAlias(keyStore, ksPass, (PredefinedAliasPrivateKeyStrategy) keyStrategy,
          trustStore, trustStrategy);
    } else {
  ...
  private static SSLContext getCachedSslContextForPredefinedAlias(final KeyStore keyStore, final String ksPass,
      final PredefinedAliasPrivateKeyStrategy keyStrategy, final KeyStore trustStore, final TrustStrategy trustStrategy)
      throws Exception {
    ...
    if (cacheElement == null) {
      ...
      synchronized (ehcache) {
        ...
        if (cacheElement == null) {
          sslContext = createSSLContextBuilder(keyStore, ksPass, keyStrategy, trustStore, trustStrategy).build();
  ...
  private static SSLContextBuilder createSSLContextBuilder(final KeyStore keyStore, final String ksPass,
      final PrivateKeyStrategy keyStrategy, final KeyStore trustStore, TrustStrategy trustStrategy) throws Exception {
  ...
  public static RestTemplate createSslRestTemplate(final KeyStore keyStore, final String ksPass,
      ...
      final Integer readTimeout, final Integer connectionTimeout, final boolean allowCached) throws Exception {
