...
public class RegistrationRestClientHttps implements RegistrationClient {
  ...
  public void initialize() {
    ...
    try {
      restTemplate = CryptographyUtils.createSslRestTemplate(this.keyStoreService.getUnderlyingKeyStore(),
          this.keyStoreService.getPassword(), this.privateKeyStrategy, this.trustStoreService.getUnderlyingKeyStore(),
          this.trustStrategy, this.readTimeout, this.connectionTimeout, false);
    } catch (Exception e) {
  ...
  public String register(final String remoteNodeUri) throws IOException {
    ...
    ResponseEntity<String> result = restTemplate.exchange(remoteNodeUri + registrationQuery, HttpMethod.POST, entity,
        String.class);
