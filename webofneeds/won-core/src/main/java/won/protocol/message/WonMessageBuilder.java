...
public class WonMessageBuilder {
    ...
    /**
     * Builds a WonMessage by adding data to the specified dataset. The dataset may be null or empty.
     * 
     * @param dataset
     * @return
    ...
    public WonMessage build(final Dataset dataset) throws WonMessageBuilderException {
        ...
        if (!injectIntoConnections.isEmpty()) {
            injectIntoConnections.forEach(receiver -> messageEventResource
                    .addProperty(WONMSG.HAS_INJECT_INTO_CONNECTION, envelopeGraph.getResource(receiver.toString())));
        }
        ...
        if (isResponseToMessageURI != null) {
            if (wonMessageType != WonMessageType.SUCCESS_RESPONSE
                ...
                throw new IllegalArgumentException(
                        "isResponseToMessageURI can only be used for SUCCESS_RESPONSE and " + "FAILURE_RESPONSE types");
            }
        ...
        for (URI contentURI : contentMap.keySet()) {
            ...
            if (signatureGraph != null) {
                 ...
                 * dataset).toString(); //the signature refers to the name of the other graph. We changed that name //so
                 * we have to replace the resource referencing it, too: signatureGraph =
                 * RdfUtils.replaceResource(signatureGraph.getResource(contentURI.toString()),
                 * signatureGraph.getResource(uniqueContentUri)); dataset.addNamedModel(uniqueContentUri,
                 * signatureGraph);
                 */
            }
    ...
    /**
     * Adds the complete message content to the message that will be built, referencing toWrap's envelope in the
     * envelope of the new message. The message that will be built has the same messageURI as the wrapped message.
     *
     * @param
     * @return
     */
    ...
    public static WonMessageBuilder setMessagePropertiesForOpen(URI messageURI, URI localConnection, URI localNeed,
            URI localWonNode, URI remoteConnection, URI remoteNeed, URI remoteWonNode, Optional<URI> remoteFacet,
        ...
        WonMessageBuilder builder = new WonMessageBuilder(messageURI)
                ...
                .setReceiverURI(remoteConnection).setReceiverNeedURI(remoteNeed).setReceiverNodeURI(remoteWonNode);
    ...
    public static WonMessageBuilder setMessagePropertiesForOpen(URI messageURI, WonMessage connectToReactTo,
        ...
        return setMessagePropertiesForOpen(messageURI, connectToReactTo.getReceiverURI(),
                ...
                connectToReactTo.getSenderNodeURI(), Optional.of(connectToReactTo.getSenderFacetURI()), welcomeMessage);
    }
    ...
    public static WonMessageBuilder setMessagePropertiesForClose(URI messageURI, WonMessageDirection direction,
        ...
        return new WonMessageBuilder(messageURI).setWonMessageDirection(direction)
                .setWonMessageType(WonMessageType.CLOSE).setSenderURI(localConnection).setSenderNeedURI(localNeed)
                .setSenderNodeURI(localWonNode).setReceiverURI(remoteConnection).setReceiverNeedURI(remoteNeed)
                .setReceiverNodeURI(remoteWonNode).setTextMessage(farewellMessage).setSentTimestampToNow();
    }
    ...
    /**
     * Sets the MessageProperties for Closing open connections (happens when the need is closed and the system is
     * closing all the corresponding connections when no connection is present from the remoteNeed
     * 
     * @param messageURI
    ...
    public static WonMessageBuilder setMessagePropertiesForDeactivateFromOwner(URI messageURI, URI localNeed,
        ...
        return new WonMessageBuilder(messageURI).setWonMessageDirection(WonMessageDirection.FROM_OWNER)
                .setWonMessageType(WonMessageType.DEACTIVATE).setSenderNeedURI(localNeed).setSenderNodeURI(localWonNode)
                .setReceiverNeedURI(localNeed).setReceiverNodeURI(localWonNode).setSentTimestampToNow();
    }
    ...
    /**
     * Sets message properties for sending a 'need message' from System to Owner, i.e. a notification from the node to
     * the owner. This message will have no effect on need or connection states and it is expected that a payload (e.g.
     * via setTextMessage()) is added to the message builder prior to calling the build() method.
     * 
     * @param messageURI
     * @param localNeed
     * @param localWonNode
    ...
    public static WonMessageBuilder setMessagePropertiesForConnect(URI messageURI, Optional<URI> localFacet,
        ...
        WonMessageBuilder builder = new WonMessageBuilder(messageURI)
                .setWonMessageDirection(WonMessageDirection.FROM_OWNER).setWonMessageType(WonMessageType.CONNECT)
                .setSenderNeedURI(localNeed).setSenderNodeURI(localWonNode).setReceiverNeedURI(remoteNeed)
    ...
    public static WonMessageBuilder setMessagePropertiesForCreate(URI messageURI, URI needURI, URI wonNodeURI) {
        return new WonMessageBuilder(messageURI).setWonMessageDirection(WonMessageDirection.FROM_OWNER)
                .setWonMessageType(WonMessageType.CREATE_NEED).setSenderNeedURI(needURI).setSenderNodeURI(wonNodeURI)
                .setReceiverNodeURI(wonNodeURI).setSentTimestampToNow();
    }
    ...
    public static WonMessageBuilder setMessagePropertiesForHint(URI messageURI, URI needURI, Optional<URI> needFacetURI,
        ...
        WonMessageBuilder builder = new WonMessageBuilder(messageURI)
                ...
                .setWonMessageType(WonMessageType.HINT_MESSAGE).setSenderNodeURI(matcherURI).setReceiverNeedURI(needURI)
    ...
    public static WonMessageBuilder setMessagePropertiesForHintFeedback(URI messageURI, URI connectionURI, URI needURI,
        ...
        return new WonMessageBuilder(messageURI).setWonMessageDirection(WonMessageDirection.FROM_OWNER)
                ...
                .setSenderNeedURI(needURI).setSenderURI(connectionURI).setSentTimestampToNow().addContent(contentModel);
    }

    public static WonMessageBuilder setMessagePropertiesForConnectionMessage(URI messageURI, URI localConnection,
            URI localNeed, URI localWonNode, URI remoteConnection, URI remoteNeed, URI remoteWonNode, Model content) {
    ...
    /**
     * Adds the complete message content to the message that will be built, referencing toForward's envelope in the
     * envelope of the new message.
     *
     * @param
     * @return
     */
    ...
    /**
     * The message to remote node will contain envelope with sentTimestamp, remoteMessageUri, direction, as well as
     * exact copy of the local message envelopes and contents.
     *
    ...
    public static WonMessageBuilder setPropertiesForNodeResponse(WonMessage originalMessage, boolean isSuccess,
        ...
        WonMessageBuilder messageBuilder = new WonMessageBuilder(messageURI)
                .setWonMessageType(isSuccess ? WonMessageType.SUCCESS_RESPONSE : WonMessageType.FAILURE_RESPONSE);
        ...
        if (WonMessageDirection.FROM_EXTERNAL == origDirection) {
            ...
            messageBuilder.setSenderNodeURI(originalMessage.getSenderNodeURI())
                    .setSenderNeedURI(originalMessage.getSenderNeedURI()).setSenderURI(originalMessage.getSenderURI());
        }
        messageBuilder.setReceiverNeedURI(originalMessage.getSenderNeedURI())
                .setReceiverNodeURI(originalMessage.getSenderNodeURI()).setReceiverURI(originalMessage.getSenderURI())
    ...
    /**
     * Adds the specified content graph, and the specified signature graph, using the specified contentURI as the graph
     * name. The contentURI will be made unique inside the message dataset by appending characters at the end.
     * 
     * @param content
     * @return
     */
    ...
    /**
     * Adds all graphs in the specified dataset as content graphs to the message. In this process, unique graph URIs are
     * generated for all graphs in the dataset, including the default graph (if present). If graphs are referenced
     * within the dataset through a triple in which the graph uri is the object, all such references are changed to
     * refer to the newly generated graph uri.
     * 
     * @param dataset
     * @return
     */
    ...
    /**
     * Retrieves one of the possibly multiple Models that does not have a signature yet. If there is none (all are
     * signed or none is found at all), a new model is created, added to the internal contentMap and returned here.
     *
     */
    ...
    /**
     * Adds a won:hasTextMessage triple to one of the unsigned content graphs in this builder. Creates a new unsigned
     * content graph if none is found.
     * 
     * @param textMessage
     *            may be null in which case the builder is not modified
     * @return
     */
     ...
     *
     * Note that this does not copy the original envelope graph, only the standard envelope properties.
     *
     * @param wonMessage
     * @return
     */
    public static WonMessageBuilder copyEnvelopeFromWonMessage(final WonMessage wonMessage) {
        WonMessageBuilder builder = new WonMessageBuilder(wonMessage.getMessageURI())
                ...
                .setReceiverNeedURI(wonMessage.getReceiverNeedURI()).setReceiverNodeURI(wonMessage.getReceiverNodeURI())
                .setSenderURI(wonMessage.getSenderURI()).setSenderNeedURI(wonMessage.getSenderNeedURI())
    ...
    public static WonMessage forwardReceivedNodeToNodeMessageAsNodeToNodeMessage(final URI newMessageUri,
        ...
        WonMessageBuilder builder = new WonMessageBuilder(newMessageUri).setWonMessageType(wonMessage.getMessageType())
                .forward(wonMessage).setForwardedMessageURI(wonMessage.getMessageURI()).setSenderNeedURI(needURI)
                .setSenderURI(connectionURI).setSenderNodeURI(wonNodeUri).setSentTimestamp(System.currentTimeMillis())
                .setReceiverURI(remoteConnectionURI).setReceiverNeedURI(remoteNeedURI)
