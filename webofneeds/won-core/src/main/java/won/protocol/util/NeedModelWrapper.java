 ...
 * It provides abstraction for the need structure of is/seeks content nodes that
 * are part of the need model. It can be used to load and query an existing need
 * dataset (or models). Furthermore it can be used to create a need model by
 * adding triples.
 * <p>
...
public class NeedModelWrapper {
   ...
   *
   * @param ds               need dataset to load
   * @param addDefaultGraphs if this is set to true a needModelGraph and a
  ...
  /**
   * Load the need and sysinfo models, if one of these models is null then
  ...
  /**
   * Add a facet. The facetURI must be a fragment URI off the need URI, i.e.
   * [needuri]#facetid, or it is just a fragment identifier, which will be
   * interpreted relative to the needURI, i.e. #facetid -> [needuri]#facetid.
   *
  ...
  public ZonedDateTime getCreationDate() {
    ...
    String dateString = RdfUtils
        .findOnePropertyFromResource(getSysInfoModel(), getNeedNode(NeedGraphType.SYSINFO), DCTerms.created).asLiteral()
        .getString();
  ...
  public String getWonNodeUri() {
    return RdfUtils.findOnePropertyFromResource(getSysInfoModel(), getNeedNode(NeedGraphType.SYSINFO), WON.HAS_WON_NODE)
        .asResource().getURI();
  }
  ...
  /**
   * get all seeks, seeks_seeks node including the resource for the needcontent
   ...
   *                           by https://tools.ietf.org/html/bcp47. The language
   *                           tag must be well-formed according to section 2.2.9
   *                           of https://tools.ietf.org/html/bcp47.
   * @return the string value or null if nothing is found
   */
  ...
  private boolean isSplittableNode(RDFNode node) {
    return node.isResource() && (node.isAnon()
        || (node.asResource().getURI().startsWith(getNeedUri()) && (!node.asResource().getURI().equals(getNeedUri()))));
  }
  ...
  /**
   * Returns a copy of the model in which no node reachable from the need node has
   ...
   * outgoing edges of the original are replicated in the copies.
   * <p>
   * Nodes that were newly introduced by this algorithm are never split.
   * <p>
   * In that special case that the graph contains a circle, the resulting graph
   * still contains a circle, and possibly one or more nodes with more than one
   * incoming edge.
   *
   * @return
   */
   ...
   * discovered further down the tree.
   * <p>
   * When a copy of the node is made, the subgraph reachable from the node is
   * copied as well.
   * <p>
   * This process is done when coming back from a depth-first recursion, i.e.
   * smaller subgraphs are copied before larger subgraphs.
   *
   * @param node
