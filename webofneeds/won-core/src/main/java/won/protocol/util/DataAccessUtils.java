...
public class DataAccessUtils {
  ...
  public static Need loadNeed(NeedRepository needRepository, final URI needURI) throws NoSuchNeedException {
    ...
    if (needs.size() > 1)
      throw new IllegalStateException(
          MessageFormat.format("Inconsistent database state detected: multiple needs found with URI {0}", needURI));
    return needs.get(0);
  }
  ...
  public static WonNode loadWonNode(WonNodeRepository repository, final URI wonNodeURI) {
    ...
    if (nodes.size() > 1)
      throw new IllegalStateException(
          MessageFormat.format("Inconsistent database state detected: multiple needs found with URI {0}", wonNodeURI));
    return nodes.get(0);
  }
  ...
  public static String loadOwnerApplication(OwnerApplicationRepository ownerApplicationRepository,
    ...
    if (ownerApplications.size() > 1)
      throw new IllegalStateException(MessageFormat
          .format("Inconsistent database state detected: multiple connections found with URI {0}", ownerApplicationId));
  ...
  public static Connection loadConnection(ConnectionRepository connectionRepository, final Long id)
    ...
    if (connections.size() > 1)
      throw new IllegalStateException(
          MessageFormat.format("Inconsistent database state detected: multiple connections found with URI {0}", id));
  ...
  public static Connection loadConnection(ConnectionRepository connectionRepository, final URI connectionURI)
    ...
    if (connections.size() > 1)
      throw new IllegalStateException(MessageFormat
          .format("Inconsistent database state detected: multiple connections found with URI {0}", connectionURI));
    return connections.get(0);
  }
