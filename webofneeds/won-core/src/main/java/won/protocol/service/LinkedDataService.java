...
public interface LinkedDataService {
    ...
    /**
     * Returns a model containing all need URIs. If page >= 0, paging is used and the respective page is returned.
     * 
     * @param page
     * @return
     */
    ...
    /**
     * Returns a model containing all need URIs that are in the specified state. If page >= 0, paging is used and the
     * respective page is returned.
     * 
     * @param page
     * @param preferedSize
    ...
    /**
     * Returns container dataset containing all connection URIs. If deep is true, the resource data of those connection
     * uris is also part of the returned resource.
     * 
     * @param deep
     * @return
     * @throws NoSuchConnectionException
     *             only in case deep is set to true and connection data for a member connection uri cannot be retrieved.
     */
     ...
     *            modification date
     * @param deep
     *            If deep is true, the resource data of those connection uris is also part of the returned resource.
     * @return
     * @throws NoSuchConnectionException
     */
    ...
    /**
     * Returns a resource containing connection uris at given page. If deep is true, the resource data of those
     * connection uris is also part of the returned resource. If page >0, paging is used and the respective page is
     * returned.
     * 
     * @param page
     * @param preferredSize
     *            preferred number of connection uris per page (null means use default)
     * @param deep
     * @return
    ...
    /**
     * Returns a resource containing connection uris that precede (by time of their latest event activities) the given
     * connection as of state that was at the specified time.
     *
     * @param beforeConnURI
     *            a connection the preceding connections of which we are interested in
     * @param preferredSize
     *            preferred number of connection uris per page (null means use default)
     * @param timeSpot
     *            date and time that specifies the connections and events state of interest
     * @param deep
     *            if true, the resource data of those connection uris is also part of the resource
     * @return
     * @throws NoSuchNeedException
     * @throws NoSuchConnectionException
     *             only in case deep is set to true and connection data for a member connection uri cannot be retrieved.
    ...
    /**
     * Returns a resource containing connection uris that follow (by time of their latest event activities) the given
     * connection as of state that was at the specified time.
     *
     * @param afterConnURI
     *            a connection the following connections of which we are interested in
     * @param preferredSize
     *            preferred number of connection uris per page (null means use default)
     * @param timeSpot
     *            date and time that specifies the connections and events state of interest
     ...
     * @param needURI
     * @param addMetadata
     *            - if true, a metadata graph is added to the dataset containing counts by connection state
     * @param deep
    ...
    /**
     * Returns paged resource containing all connection uris belonging to the specified need.
     *
     * @param page
     *            number
     * @param preferredSize
     ...
     * @param messageType
     *            the event type that should be used for defining connection latest activity; null => all event types
     * @param timeSpot
    ...
    /**
     * Returns paged resource containing all connection uris belonging to the specified need that precede the given
     * connection URI from the point of view of their latest events.
     *
     * 
     * @param needURI
     *            local need the connections of which are retrieved
     * @param resumeConnURI
     *            the returned slice connections precede (in time of their latest events) this connection uri
     * @param preferredSize
     *            preferred number of connection uris per page (null means use default)
     * @param messageType
     *            the event type that should be used for defining connection latest activity; null => all event types
    ...
    /**
     * Returns paged resource containing all connection uris belonging to the specified need that follows the given
     * connection URI from the point of view of their latest events.
     *
     * @param needURI
     *            local need the connections of which are retrieved
     * @param resumeConnURI
     *            the returned slice connections follow (in time of their latest events) this connection uri
     * @param preferredSize
     *            preferred number of connection uris per page (null means use default)
     * @param messageType
     *            the event type that should be used for defining connection latest activity; null => all event types
    ...
    /**
     * Returns a dataset describing the need with the specified URI. If the need is in state ACTIVE, 'deep' data is
     * added if requested.
     * 
     * @param needUri
     * @param deep
     *            - include need's connections datasets and each connection's events' datasets
     * @param deepLayerSize
     *            - number of connections and events to include in the deep need dataset
     * @return
     * @throws NoSuchNeedException
     */
    ...
    /**
     * Returns paged resource containing all event uris belonging to the specified connection. If deep is true, the
     * event dataset is added to the result.
     * 
     * @param connectionUri
    ...
    /**
     * returns a dataset of the (message) event with the specified URI, with a value that can be used for an etag. If
     * the current etag is the same as the specified one, the dataset is null, indicating no change.
     * 
     * @param eventURI
     */
     ...
     * after the specified lastSeenMessageURI, or the total message count and respective timestamps for a connection for
     * which no lastSeenMessageURI is specified.
     * 
     * @param needURI
