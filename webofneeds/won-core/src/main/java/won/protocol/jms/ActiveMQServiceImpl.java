...
public class ActiveMQServiceImpl implements ActiveMQService {
  ...
  public final String getProtocolQueueNameWithResource(URI resourceUri) {
    ...
    try {
      ...
      activeMQOwnerProtocolQueueName = RdfUtils
          .getStringPropertyForPropertyPath(linkedDataSource.getDataForResource(wonNodeUri), wonNodeUri, path);
  ...
  public Set<String> getMatcherProtocolTopicNamesWithResource(URI resourceURI) {
    ...
    for (int i = 0; i < matcherProtocolTopicList.size(); i++) {
      try {
        ...
        activeMQMatcherProtocolTopicNames.add(RdfUtils
            .getStringPropertyForPropertyPath(linkedDataSource.getDataForResource(resourceURI), resourceURI, path));
      } catch (HttpClientErrorException e) {
        if (e.getStatusCode() == HttpStatus.NOT_FOUND) {
          return null;
        } else
