  ...
  private ValidationResult validateSelect(final Dataset input) {
    try (QueryExecution qe = QueryExecutionFactory.create(constraint, input)) {
      ...
      while (result.hasNext()) {
        ...
        if (node != null) {
          ...
          if (node.isLiteral()) {
            ...
            if (SELECT_VALIDATION_PASSED_VALUE.equals(resultString)) {
              ...
              return new ValidationResult(false, "SELECT query produced this binding: " + binding.toString());
            }
          }
        } else {
          ...
          // ValidationResult
          String errorMessage = "SPARQL query produced this solution, which indicates a problem: " + binding.toString()
              + ", query: " + constraint.toString(Syntax.syntaxSPARQL_11);
          if (result.hasNext()) {
        ...
        throw new IllegalStateException("We should have returned a result earlier. Bindings: " + binding.toString()
            + ", Constraint: " + constraint.toString(Syntax.syntaxSPARQL_11));
      }
