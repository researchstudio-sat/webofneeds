...
public class RdfDatasetAttachmentConverter extends AbstractHttpMessageConverter<Dataset> {
  ...
  private static final MediaType[] supportedMediaTypes = { MediaType.ALL // we can do this because we have placed a hack
                                                                         // in the LinkedDataWebController that
      // looks into the dataset and determines the concrete MediaType of the
      // base64-encoded content in the
      // dataset. Thus, spring's content negotiation will choose this class as a
      // compatible converter and the
  ...
  protected void writeInternal(Dataset dataset, HttpOutputMessage httpOutputMessage)
    ...
    // TODO: here, we decode the base64 content only to have it encoded again by the
    // framework. Can we tell the webMVC framework not to encode in base64 somehow?
