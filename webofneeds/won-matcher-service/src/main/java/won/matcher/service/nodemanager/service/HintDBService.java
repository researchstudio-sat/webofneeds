 ...
 *
 * Currently the hint "database" is just implemented as an in memory bloom filter so that (best guess) duplicate checks
 * are possible to avoid sending duplicate hints. In a future version this might be changed to a persistent hint
 * database.
 */
@Component
@Scope("singleton")
