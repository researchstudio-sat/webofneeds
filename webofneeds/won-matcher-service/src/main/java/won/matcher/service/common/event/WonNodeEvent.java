...
/**
 * This event is used in the matching service to indicate status about won nodes. Usually first a new won node is
 * discovered, then some component can decide to connect to it or skip processing this won node. If we are connected to
 * a won node, we know not only the uri but also the whole won node info.
 *
