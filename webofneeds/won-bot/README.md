# Bots
 A bot is a so-called 'owner application'. That is, it communicates with WoN nodes and creates and manages needs on them. It also acts on their behalf, i.e., causes them to send messages to their connected needs.
...
* whenever a message is received for one of the needs controlled by the bot.
...
* The [NeedProducer](src/main/java/won/bot/framework/component/needproducer/NeedProducer.java) obtained via `EventListenerContext.getNeedProducer()`. It's a facility for creating needs, much like an iterator. Depending on the configured implementation, data can be read from any source and transformed into a need. To see how this is done in practice, look at the [MailFileNeedProducer](src/main/java/won/bot/framework/component/needproducer/impl/MailFileNeedProducer.java)
...
For example, this is how [a bot creates a Need](src/main/java/won/bot/framework/eventbot/action/impl/needlifecycle/AbstractCreateNeedAction.java):
```
//assuming 
//   * wonNodeURI contains a valid WonNodeURI,
//   * needURI has been newly created and conforms to the WoN node's URI pattern 
WonMessageBuilder builder = WonMessageBuilder
 .setMessagePropertiesForCreate(
      wonNodeInformationService.generateEventURI(wonNodeURI),              // generate an URI for the 'create Need' event 
      needURI,                                                             // pass the new Need URI
      wonNodeURI)                                                          // pass the WoN node URI
      .addContent(needModel, null);                                        // add the Need's content
...
## Creating Needs
Needs are created by sending a specific message. If using the dedicated [CreateNeedsWithFacetsAction](src/main/java/won/bot/framework/eventbot/action/impl/needlifecycle/CreateNeedWithFacetsAction.java), most of the boilerplate code is taken care of. If the bot is meant to publish an existing collection of resources, e.g. files or database objects in the form of needs, it's a question of configuring the `NeedProducer` to access that data and produce a Jena RDF Model that represents a need. (For an example, look at the [MailFileNeedProducer](src/main/java/won/bot/framework/component/needproducer/impl/MailFileNeedProducer.java)

## How to Learn Writing Bots
A good starting point for understanding the framework is the [EchoBot](src/main/java/won/bot/impl/EchoBot.java) and its corresponding [EchoBotApp](src/main/java/won/bot/app/EchoBotApp.java). This bot creates one need at startup, and it registers with the WoN node configured in its [node-uri-source.properties](/webofneeds/conf/node-uri-source.properties) file so it is always notified when a new need is created. When that happens, the bot attempts to establish a connection with the new need and if the new need accepts it, the bot echoes any text message received from the new need.

For an example of a bot for creating needs, the [NeedCreatorBot](src/main/java/won/bot/impl/NeedCreatorBot.java) is a good model. This bot reads need `Model`s from the configured `NeedProducer` and creates new needs on the configured WoN nodes. It does that until the `NeedProducer` is exhausted.
