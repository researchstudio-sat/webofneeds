    ...
    public static class MyBot extends CommentBot {
        ...
        public void executeCommentRDFValidationAssert() {
            ...
            // "GRAPH ?g1 {" +
                    ...
                    // "} ."+
                    // "Graph ?g4 {" +
                    "?connection won:hasFacet won:CommentFacet." + "?connection won:hasRemoteConnection ?connection2."
        ...
        public Dataset executeNeedRDFValidationAsserts() {
            ...
            String queryString = sparqlPrefix + "SELECT ?need WHERE {" + "?need a won:Need" + "}";
            Query query = QueryFactory.create(queryString);
            ...
            // String expected1 = "( ?event = <" + EXAMPLE_ONTOLOGY_URI + "Open_01_1> ) ( ?eventType = <" +
            // WON_ONTOLOGY_URI + "Open> )";
        ...
        public void executeNeedCommentConnectionRDFValidationAsserts(Model needModel, Model commentModel) {
            ...
            System.out.println("executing queries...");
            String queryString = sparqlPrefix + "SELECT ?need ?connection ?state WHERE {"
            ...
            Dataset needConnections = getEventListenerContext().getLinkedDataSource()
                    .getDataForResource(needConnectionCollectionURI);
            ...
            Dataset needConnection = getEventListenerContext().getLinkedDataSource()
                    .getDataForResource(needConnectionURI);
            String queryString3 = sparqlPrefix + "SELECT ?remoteConnection WHERE {"
            ...
            Dataset remoteConnections = getEventListenerContext().getLinkedDataSource()
                    .getDataForResource(commentConnectionsURI);
            String queryString4 = sparqlPrefix + "SELECT ?remoteConnection WHERE {"
                    + "?connection won:hasRemoteConnection ?remoteConnection" + "}";
