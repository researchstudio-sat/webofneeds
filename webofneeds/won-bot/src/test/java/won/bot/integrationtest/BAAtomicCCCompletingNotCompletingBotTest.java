...
import won.bot.impl.BAAtomicCCCompletingNotCompletingBot;
...
public class BAAtomicCCCompletingNotCompletingBotTest {
     ...
     */
    @Test
    public void testBAAtomicCCCompletingNotCompletingBot() throws Exception {
        // now check the results!
        logger.info("start test case testBAAtomicCCCompletingNotCompletingBot");
        bot.executeAsserts();
        logger.info("finishing test case testBAAtomicCCCompletingNotCompletingBot");
    }

    @Test
    public void testBAAtomicCCCompletingNotCompletingRDF() {
        logger.info("starting test case testBAAtomicCCCompletingNotCompletingRDF");
        bot.executeRDFValidationAssert();
        logger.info("finishing test case testBAAtomicCCCompletingNotCompletingRDF");
    }

    @Test
    public void testBAAtomicCCCompletingNotCompletingBAStateRDF() {
        logger.info("starting test case testBAAtomicCCCompletingNotCompletingBAStateRDF");
        bot.executeBAStateRDFValidationAssert();
        logger.info("finishing test case testBAAtomicCCCompletingNotCompletingBAStateRDF");
    }
     ...
     */
    public static class MyBot extends BAAtomicCCCompletingNotCompletingBot {
        /**
        ...
        public void executeRDFValidationAssert() {
            ...
            Dataset dataModel = linkedDataSource.getDataForResourceWithPropertyPath(needs.get(0),
                    PropertyPathConfigurator.configurePropertyPaths(), 300, 4, true);
            logger.debug("crawled dataset: {}", RdfUtils.toString(dataModel));

            String queryString = sparqlPrefix + "SELECT ?need ?connection ?need2 WHERE {"
                    ...
                    + "?connection won:hasRemoteConnection ?connection2." + "?connection2 won:belongsToNeed ?need2."
        ...
        public void executeBAStateRDFValidationAssert() {
            ...
            String queryString = sparqlPrefix + "SELECT ?need ?connection ?need2 WHERE {"
                    + "?need won:hasConnections ?connections." + "?connections rdfs:member ?connection."
                    + "?connection won:hasFacet won:BAAtomicCCCoordinatorFacet."
                    + "?connection  wontx:hasBAState wontx:Ended." + "?connection won:hasRemoteConnection ?connection2."
                    + "?connection2 won:belongsToNeed ?need2." + "?connection2 won:hasFacet won:BACCParticipantFacet. "
