...
public class GroupingBot extends EventBot {
    ...
    protected void initializeEventListeners() {
        ...
        // make a composite filter, with one filter for each autoResponder that wait for the FinishedEvents the
        // responders emit.
        ...
        this.autoResponderCreator = new ActionOnEventListener(ctx, groupMemberFilter, new BaseEventBotAction(ctx) {
            ...
            protected void doRun(final Event event, EventListener executingListener) throws Exception {
                // create a listener that automatically answers messages, only for that need URI. We let it send
                // NO_OF_MESSAGES messages
                                ...
                                if (event instanceof MessageFromOtherNeedEvent) {
                                    ...
                                    logger.debug(
                                            ...
                                            new Object[] { event.toString(), msg.getWonMessage().getMessageURI(),
                                                    ...
                                                    msg.getRemoteNeedURI(), msg.getNeedURI() });
                                }
                            }
                        });
        ...
        this.groupCreator = new ActionOnceAfterNEventsListener(ctx, "groupCreator", NO_OF_GROUPMEMBERS,
                new CreateNeedWithFacetsAction(ctx, botContextWrapper.getGroupListName(),
        ...
        this.needConnector = new ActionOnceAfterNEventsListener(ctx, "needConnector", NO_OF_GROUPMEMBERS + 1,
                new ConnectFromListToListAction(ctx, botContextWrapper.getGroupListName(),
                        botContextWrapper.getGroupMembersListName(), FacetType.GroupFacet.getURI(),
                        FacetType.ChatFacet.getURI(), MILLIS_BETWEEN_MESSAGES, "Hi from the " + "GroupingBot!"));
        bus.subscribe(NeedCreatedEvent.class, this.needConnector);

        // add a listener that is informed of the connect/open events and that auto-opens
        // subscribe it to:
        // * connect events - so it responds with open
        // * open events - so it responds with open (if the open received was the first open, and we still need to
        // accept the connection)
        this.autoOpener = new ActionOnEventListener(ctx, new OpenConnectionAction(ctx, "Hi from the GroupingBot!"));
        bus.subscribe(ConnectFromOtherNeedEvent.class, this.autoOpener);

        // now, once all connections have been opened, make 1 bot send a message to the group, the subsequent listener
        // will cause let wild chatting to begin
        ...
        this.messagesDoneListener = new ActionOnceAfterNEventsListener(ctx, "messagesDoneListener",
                ...
                new DeactivateAllNeedsOfListAction(ctx, botContextWrapper.getGroupMembersListName()));
        bus.subscribe(FinishedEvent.class, this.messagesDoneListener);

        // When the group facet need is deactivated, all connections are closed. wait for the close events and signal
        // work done.
        ...
        this.groupMemberCreator = new ActionOnEventListener(ctx, "groupMemberCreator", new CreateNeedWithFacetsAction(
                ctx, botContextWrapper.getGroupMembersListName(), FacetType.ChatFacet.getURI()), NO_OF_GROUPMEMBERS);
