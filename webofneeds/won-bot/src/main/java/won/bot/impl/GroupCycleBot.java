...
/**
 * Bot that creates NUMBER_OF_GROUPS groupchats, adds NUMBER_OF_GROUPMEMBERS members to each, links them to each other
 * and sends a message on behalf of one of the members, potentially causing an endless echo. Used for verifying that the
 * group facet suppresses echos.
 */
public class GroupCycleBot extends EventBot {
    ...
    /**
     * Creates a need with a chat facet and a group facet, then stops. Its deactivate message is the URI of the created
     * group need.
     */
                    ...
                    new CommandResultFilter(command), new BaseEventBotAction(context) {
                        ...
                        protected void doRun(Event event, EventListener executingListener) throws Exception {
                            ...
                            Optional<Object> uriMessage = resultEvent.isSuccess()
                                    ? Optional.of(resultEvent.getNeedURI()) : Optional.empty();
        ...
        protected void onActivate(Optional<Object> message) {
                        ...
                        new CommandResultFilter(command), new BaseEventBotAction(context) {
                            ...
                            protected void doRun(Event event, EventListener executingListener) throws Exception {
                                ...
                                if (resultEvent.isSuccess()) {
                                    ...
                                    context.getEventBus()
                                            .publish(new GroupMemberCreatedEvent(resultEvent.getNeedURI()));
                                }
                        ...
                        protected void doRun(Event event, EventListener executingListener) throws Exception {
                            ...
                            members.forEach(memberURI -> {
                                ...
                                ConnectCommandEvent connectCommandEvent = new ConnectCommandEvent(memberURI,
                                        groupNeedURI, FacetType.ChatFacet.getURI(), FacetType.GroupFacet.getURI(),
                                        ...
                                        new ActionOnFirstEventListener(context,
                                                ...
                                                new BaseEventBotAction(context) {
                                                    ...
                                                    protected void doRun(Event event, EventListener executingListener)
                                                        ...
                                                        logger.debug(
                                                                "sending connect to group need {} on behalf of member {}, succeeded: {}",
            ...
            subscribeWithAutoCleanup(TargetCountReachedEvent.class, new ActionOnFirstEventListener(context,
                    membersConnectedCounter.makeEventFilter(), new BaseEventBotAction(context) {
                        @Override
                        protected void doRun(Event event, EventListener executingListener) throws Exception {
                            logger.debug("finished connecting all {} members to group {} ", NUMBER_OF_GROUPMEMBERS,
                                    groupNeedURI);
                    ...
                    subscribeWithAutoCleanup(ConnectCommandResultEvent.class, new ActionOnFirstEventListener(context,
                            new CommandResultFilter(connectCommandEvent), new BaseEventBotAction(context) {
                                ...
                                protected void doRun(Event event, EventListener executingListener) throws Exception {
                                    ConnectCommandResultEvent resultEvent = (ConnectCommandResultEvent) event;
                                    logger.debug(
                                            "sending connect to group need {} on behalf of group need {}, succeeded: {}",
                                            new Object[] { connectCommandEvent.getRemoteNeedURI(),
                    ...
                    new ActionOnEventListener(context, new BaseEventBotAction(context) {
                        ...
                        protected void doRun(Event event, EventListener executingListener) throws Exception {
                            ...
                            logger.debug("need {} received message from need {}, text: '{}', message uri: {}",
                                    new Object[] { messageEvent.getNeedURI(), messageEvent.getRemoteNeedURI(),
    ...
    /**
     * Class that encapsulates a connection, and allows us to remember the one connection we want to use to send the
     * only connection message in this bot.
     */
                    ...
                    receivedMessagesCounter.makeEventFilter(), new BaseEventBotAction(context) {
                        ...
                        protected void doRun(Event event, EventListener executingListener) throws Exception {
                            ...
                            if (currentCount < targetCount) {
                                ...
                                logger.info("received group message {} of {}, target count reached", currentCount,
                                        targetCount);
                            } else {
