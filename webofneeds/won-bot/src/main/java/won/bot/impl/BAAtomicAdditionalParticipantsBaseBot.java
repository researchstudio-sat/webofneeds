...
public abstract class BAAtomicAdditionalParticipantsBaseBot extends EventBot {
    ...
    protected BAAtomicAdditionalParticipantsBaseBot() {
        ...
        this.firstPhasetestScriptListeners = Collections
                .synchronizedList(new ArrayList<BATestScriptListener>(noOfNonDelayedNeeds - 1));
        this.firstPhasetestScriptWithDelayListeners = Collections
                .synchronizedList(new ArrayList<BATestScriptListener>(noOfDelayedNeeds));
        this.secondPhasetestScriptListeners = Collections
                .synchronizedList(new ArrayList<BATestScriptListener>(noOfNeeds - 1));
    ...
    protected void initializeEventListeners() {
        ...
        WaitForNEventsListener allNeedsCreatedListener = new WaitForNEventsListener(ctx, "waitForAllNeedsCreated",
                noOfNeeds);
        ...
        this.participantNeedCreator = new ActionOnEventListener(
                ctx, "participantCreator", new CreateNeedWithFacetsAction(ctx,
                        botContextWrapper.getParticipantListName(), getParticipantFacetType().getURI()),
        ...
        this.delayedParticipantNeedCreator = new ActionOnEventListener(
                ctx, "delayedParticipantCreator", new CreateNeedWithFacetsAction(ctx,
                        botContextWrapper.getParticipantDelayedListName(), getParticipantFacetType().getURI()),
                noOfDelayedNeeds);
        ...
        this.coordinatorNeedCreator = new ActionOnEventListener(ctx, "coordinatorCreator",
                new FinishedEventFilter(participantNeedCreator), new CreateNeedWithFacetsAction(ctx,
                        botContextWrapper.getCoordinatorListName(), getCoordinatorFacetType().getURI()),
                1);
        ...
        ConnectFromListToListAction.ConnectHook scriptConnectHook = new ConnectFromListToListAction.ConnectHook() {
            ...
            public void onConnect(final URI fromNeedURI, final URI toNeedURI) {
                ...
                BATestScriptListener testScriptListener = new BATestScriptListener(ctx, firstPhasescriptIterator.next(),
                ...
                firstPhaseScriptListenerFilter
                        .addFilter(new AcceptOnceFilter(new FinishedEventFilter(testScriptListener)));
                ...
                secondPhaseScriptListenerFilter
                        .addFilter(new AcceptOnceFilter(new FinishedEventFilter(secondPhaseTestScriptListener)));
            }
        };
        ...
        ConnectFromListToListAction.ConnectHook scriptConnectWithDelayHook = new ConnectFromListToListAction.ConnectHook() {
            ...
            public void onConnect(final URI fromNeedURI, final URI toNeedURI) {
                // create the listener that will execute the script actions
                BATestScriptListener testScriptListener = new BATestScriptListener(ctx,
                        firstPhaseScriptWithDelayIterator.next(), fromNeedURI, toNeedURI, MILLIS_BETWEEN_MESSAGES);
                // remember it so we can check its state later
                ...
                // add a filter that will wait for the FinishedEvent emitted by that listener
                // wrap it in an acceptance filter to make extra sure we count each listener only once.
                firstPhaseScriptWithDelayListenerFilter
                        .addFilter(new AcceptOnceFilter(new FinishedEventFilter(testScriptListener)));
        ...
        this.needConnector = new ActionOnEventListener(ctx, "needConnector",
                new FinishedEventFilter(allNeedsCreatedListener),
                new ConnectFromListToListAction(ctx, botContextWrapper.getCoordinatorListName(),
                        botContextWrapper.getParticipantListName(), getCoordinatorFacetType().getURI(),
                        getParticipantFacetType().getURI(), MILLIS_BETWEEN_MESSAGES, scriptConnectHook, "Hi!"),
                1);
        ...
        this.needConnectorWithDelay = new ActionOnEventListener(ctx, "needConnectorWithDelay",
                allNonDelayedConnectedFilter,
                new ConnectFromListToListAction(ctx, botContextWrapper.getCoordinatorListName(),
                        botContextWrapper.getParticipantDelayedListName(), getCoordinatorFacetType().getURI(),
                        getParticipantFacetType().getURI(), MILLIS_BETWEEN_MESSAGES, scriptConnectWithDelayHook, "Hi!"),
                1);
        bus.subscribe(FinishedEvent.class, this.needConnectorWithDelay); // TODO: MAKE THIS SO
                                                                         // URI_LIST_NAME_PARTICIPANT_DELAYED
                                                                         // "delayedParticipants" works again

        // for each group member, there are 2 listeners waiting for messages. when they are all finished, we're done.
        this.firstPhaseWithDelayDoneListener = new ActionOnceAfterNEventsListener(ctx,
                "firstPhaseDoneWithDelayListener", firstPhaseScriptWithDelayListenerFilter, noOfDelayedNeeds,
                new BaseEventBotAction(ctx) {
                    ...
                    protected void doRun(final Event event, EventListener executingListener) throws Exception {
                        ...
                        Iterator<BATestScriptListener> firstPhaseWithDelayListeners = firstPhasetestScriptWithDelayListeners
                                .iterator();
                        Iterator<BATestScriptListener> combinedFirstPhaseListeners = Iterators
                                .concat(firstPhaseListeners, firstPhaseWithDelayListeners);
                        ...
                        for (BATestScriptListener listener : secondPhasetestScriptListeners) {
                            ...
                            listener.setCoordinatorSideConnectionURI(
                                    correspondingFirstPhaseListener.getCoordinatorSideConnectionURI());
                            listener.setParticipantSideConnectionURI(
                                    correspondingFirstPhaseListener.getParticipantSideConnectionURI());
                            listener.updateFilterForBothConnectionURIs();
                            bus.publish(new SecondPhaseStartedEvent(correspondingFirstPhaseListener.getCoordinatorURI(),
                                    correspondingFirstPhaseListener.getCoordinatorSideConnectionURI(),
        ...
        // for each group member, there are 2 listeners waiting for messages. when they are all finished, we're done.
        this.scriptsDoneListener = new ActionOnceAfterNEventsListener(ctx, "scriptsDoneListener",
                secondPhaseScriptListenerFilter, noOfNeeds - 1,
        ...
        this.workDoneSignaller = new ActionOnceAfterNEventsListener(ctx, "workDoneSignaller", noOfNeeds - 1,
