...
public abstract class BAAtomicBaseBot extends EventBot {
    ...
    protected BAAtomicBaseBot() {
        ...
        this.scriptListeners = Collections.synchronizedList(new ArrayList<TwoPhaseScriptListener>(noOfNeeds - 1));
    }
    ...
    protected void initializeEventListeners() {
        ...
        this.participantNeedCreator = new ActionOnEventListener(ctx, "participantCreator",
                new CreateNeedWithFacetsAction(ctx, botContextWrapper.getParticipantListName(),
                        getParticipantFacetType().getURI()),
                noOfNeeds - 1);
        ...
        ConnectFromListToListAction.ConnectHook scriptConnectHook = new ConnectFromListToListAction.ConnectHook() {
            ...
            public void onConnect(final URI fromNeedURI, final URI toNeedURI) {
                ...
                BATestScriptListener testScriptListener = new BATestScriptListener(ctx, script.getFirstPhaseScript(),
        ...
        this.firstPhaseDoneListener = new ActionOnceAfterNEventsListener(ctx, "firstPhaseDoneListener",
                firstPhaseScriptListenerFilter, noOfNeeds - 1, new BaseEventBotAction(ctx) {
                    @Override
                    protected void doRun(final Event event, EventListener executingListener) throws Exception {
                        ...
                        for (TwoPhaseScriptListener listener : scriptListeners) {
                            ...
                            listener.getSecondPhaseListener().setCoordinatorSideConnectionURI(
                                    listener.getFirstPhaseListener().getCoordinatorSideConnectionURI());
                            listener.getSecondPhaseListener().setParticipantSideConnectionURI(
                                    listener.getFirstPhaseListener().getParticipantSideConnectionURI());
                            listener.getSecondPhaseListener().updateFilterForBothConnectionURIs();
                            bus.publish(
                                    new SecondPhaseStartedEvent(listener.getFirstPhaseListener().getCoordinatorURI(),
                                            listener.getFirstPhaseListener().getCoordinatorSideConnectionURI(),
    ...
    private class TwoPhaseScriptListener {
        ...
        private TwoPhaseScriptListener(final BATestScriptListener firstPhaseListener,
                final BATestScriptListener secondPhaseListener) {
