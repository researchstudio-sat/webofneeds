...
public class DebugBotIncomingMessageToEventMappingAction extends BaseEventBotAction {
  ...
  private void referToEarlierMessages(EventListenerContext ctx, EventBus bus, Connection con, String crawlAnnouncement,
  ...
  private void reject(EventListenerContext ctx, EventBus bus, Connection con, boolean useWrongSender) {
    ...
    referToEarlierMessages(ctx, bus, con,
        ...
        state -> {
          URI uri = state
              .getLatestProposesOrClaimsMessageSentByNeed(useWrongSender ? con.getNeedURI() : con.getRemoteNeedURI());
          return uri == null ? Collections.EMPTY_LIST : Arrays.asList(uri);
    ...
    referToEarlierMessages(ctx, bus, con,
        ...
        state -> {
          return state.getNLatestMessageUris(m -> {
            ...
            return ownedNeedUri != null && ownedNeedUri.equals(m.getSenderNeedURI()) && allowOwnClauses
