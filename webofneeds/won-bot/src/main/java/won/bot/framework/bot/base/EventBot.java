 ...
 *
 * Once the bot's work is done, the workIsDone() method should be called to allow the framework to shut down the bot
 * gracefully.
 *
 * All methods from the Bot interface are converted to Events by the EventBot. Subclasses should implement the
 * initializeEventListeners() method to register their event listeners. The corresponding shutdownEventListeners() is
 * called when the bot's shutdown method is called an may be used to perform shutdown work for listeners.
 *
 * An event bot implementation should take care not to do long or blocking work in event listeners. Whenever a long
 * computation or a blocking call is done, it should be done in a Runnable, executed in the Executor (see below).
 *
 * The bot's services and other environment can be accessed in listeners through the instance of EventListenerContext.
 *
 ...
 * </ul>
 *
 * The bot will only react to onXX methods (i.e., create events and publish them on the internal event bus) if it is in
...
public abstract class EventBot extends TriggeredBot {
    ...
    public final void onMessageFromOtherNeed(final Connection con, final WonMessage wonMessage) {
        if (getLifecyclePhase().isActive()) {
            ...
            logger.info(
                    "not publishing event for call to onMessageFromOtherNeed() as the bot is not in state {} but {}",
                    BotLifecyclePhase.ACTIVE, getLifecyclePhase());
        }
    }
    ...
    public void onConnectFromOtherNeed(final Connection con, final WonMessage wonMessage) {
        if (getLifecyclePhase().isActive()) {
            ...
            logger.info(
                    "not publishing event for call to onConnectFromOtherNeed() as the bot is not in state {} but {}",
                    BotLifecyclePhase.ACTIVE, getLifecyclePhase());
        }
    }
    ...
    /*
     * Override this method to initialize your event listeners. Will be called before the first event is published.
     */
    protected abstract void initializeEventListeners();

    /*
     * Override this method to shut down your event listeners. Will be called after the last event is published. Event
     * listeners may receive delayed events after this method is called.
     */
    ...
    /**
     * Init method used to create the event bus and allow event listeners to register. Do not override! It not final to
     * allow for CGLIB autoproxying.
     */
    @Override
    ...
    /**
     * Init method used to shut down the event bus and allow event listeners shut down. Do not override! It not final to
     * allow for CGLIB autoproxying.
     */
    @Override
    ...
    /**
     * Event listener that will stop the bot by publishing a WorkDoneEvent if an ErrorEvent is seen. Expects to be
     * registered for WorkDoneEvents and ErrorEvents and will not react to a WorkDoneEvent.
     */
