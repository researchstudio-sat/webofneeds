...
public class TelegramHelpAction extends BaseEventBotAction {
    ...
    protected void doRun(Event event, EventListener executingListener) throws Exception {
        if (event instanceof SendHelpEvent) {
             ...
             * 
             * Message message = update.getMessage(); Long chatId = message.getChatId();
             * 
             * ReplyKeyboardMarkup replyKeyboardMarkup = new ReplyKeyboardMarkup(); List<KeyboardRow> commands = new
             * ArrayList<>(); KeyboardRow kb = new KeyboardRow();
             * 
             * kb.add("create need"); KeyboardRow kb1 = new KeyboardRow(); kb1.add("list your needs");
             * 
             * commands.add(kb); commands.add(kb1);
             * 
             * replyKeyboardMarkup.setResizeKeyboard(true); replyKeyboardMarkup.setKeyboard(commands);
             * 
             * 
             * SendMessage sendMessage = new SendMessage(); sendMessage.setChatId(chatId);
             * sendMessage.setText("how can i help you?"+message.getText());
             * sendMessage.setReplyMarkup(replyKeyboardMarkup);
             * 
             * try{ wonTelegramBotHandler.sendMessage(sendMessage); }catch (TelegramApiException e){
