...
public class MailCommandAction extends BaseEventBotAction {
    ...
    private void processReferenceMailCommands(MimeMessage message, WonURI wonUri) {
        ...
        try {
            ...
            case IMPLICIT_OPEN_CONNECTION:
                bus.publish(
                        new ConnectCommandEvent(needUri, remoteNeedUri, mailContentExtractor.getTextMessage(message)));
                break;
            case SENDMESSAGE:
                con = RdfUtils.findFirst(connectionRDF, x -> new ConnectionModelMapper().fromModel(x));
            ...
            logger.error(
                    "ACCESS RESTRICTION: sender of original and command mail are not equal, command will be blocked");
        } catch (Exception e) {
     ...
     * 
     * @param message
     *            used to extract sender adress and subject(title)
     * @return
     */
    ...
    private ActionType determineAction(EventListenerContext ctx, MimeMessage message, WonURI wonUri) {
        try {
            ...
            switch (wonUri.getType()) {
            case CONNECTION:
                boolean connected = WonRdfUtils.ConnectionUtils
                        .isConnected(ctx.getLinkedDataSource().getDataForResource(wonUri.getUri()), wonUri.getUri());
                if (ActionType.CLOSE_CONNECTION.equals(mailAction)) {
                    ...
                    // if the connection is not connected yet and we do not parse any command we assume that the
                    // mailsender wants to establish a connection
