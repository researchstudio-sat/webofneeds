...
/**
 * Listener used to execute a business activity test script. It knows the URIs of one participant and one coordinator
 * and sends messages on behalf of these two as defined by the script it is given.
 *
...
public class BATestScriptListener extends AbstractFinishingListener {
    ...
    public boolean isFinished() {
        synchronized (countMonitor) {
            ...
            logger.debug("isFinished()=={}, scripts.hasNext()=={}, messagesInFlight =={}",
                    new Object[] { bFinished, script.hasNext(), messagesInFlight });
            return bFinished;
        }
    }
    ...
    protected void handleEvent(final Event event) throws Exception {
        ...
        if (this.script.hasNext()) {
            ...
            if (action.isNopAction()) {
                ...
                sendMessage(action, fromCon, fromNeed, toCon, toNeed,
                        new Date(System.currentTimeMillis() + millisBetweenMessages));
                synchronized (countMonitor) {
                    this.messagesInFlight++;
                }
            }
        ...
        try (QueryExecution qExec = QueryExecutionFactory.create(query, dataModel, binding)) {
            ...
            if (!res.hasNext()) {
                throw new IllegalStateException(
                        "connection state of connection " + fromCon + " does " + "not allow next action " + action
                                + ". Could not determine actual connection state: not found");
            }
    ...
    private void sendMessage(final BATestScriptAction action, final URI fromCon, final URI fromNeed, final URI toCon,
            final URI toNeed, Date when) throws Exception {
            ...
            public void run() {
                try {
                    getEventListenerContext().getWonMessageSender().sendWonMessage(createWonMessageForConnectionMessage(
                            fromCon, fromNeed, toCon, toNeed, action.getMessageToBeSent()));
                } catch (Exception e) {
    ...
    private WonMessage createWonMessageForConnectionMessage(URI fromConUri, URI fromNeedUri, URI toConUri,
            URI toNeedUri, Model content) throws WonMessageBuilderException {
        ...
        return WonMessageBuilder
                .setMessagePropertiesForConnectionMessage(wonNodeInformationService.generateEventURI(localWonNode),
                        fromConUri, fromNeedUri, localWonNode, toConUri, toNeedUri, remoteWonNode, content)
                .build();
    }
    ...
    private WonMessage createWonMessageForOpen(URI fromConUri, URI fromNeedUri, URI toConUri, URI toNeedUri)
        ...
        return WonMessageBuilder.setMessagePropertiesForOpen(wonNodeInformationService.generateEventURI(localWonNode),
                fromConUri, fromNeedUri, localWonNode, toConUri, toNeedUri, remoteWonNode, null).build();
    }
    ...
    /**
     * Checks whether the event is really about the connection between coordinator and participant by fetching the
     * linked data description for the connection and checking the remoteNeed URI.
     */
    ...
    private void rememberConnectionURI(final URI needURI, final URI connectionURI) {
        if (this.coordinatorURI.equals(needURI)) {
            ...
            throw new IllegalStateException(new MessageFormat("Listener called for need {0}, "
                    + "which is neither my coordinator {1} nor my " + "participant {2}")
                            .format(new Object[] { needURI, this.coordinatorURI, this.participantURI }));
        }
    }
    ...
    private boolean isConnectionURIKnown(URI connectionURI) {
        assert connectionURI != null : "connectionURI must not be null";
        return connectionURI.equals(this.coordinatorSideConnectionURI)
                || connectionURI.equals(this.participantSideConnectionURI);
    }
    ...
    public String toString() {
        return "BATestScriptListener" + "{" + "name=" + name + ", coordinatorURI=" + coordinatorURI
                + ", participantURI=" + participantURI + ", coordinatorSideConnectionURI="
                + coordinatorSideConnectionURI + ", participantSideConnectionURI=" + participantSideConnectionURI
                + ", messagesInFlight=" + messagesInFlight + ", millisBetweenMessages=" + millisBetweenMessages + '}';
    }
}
