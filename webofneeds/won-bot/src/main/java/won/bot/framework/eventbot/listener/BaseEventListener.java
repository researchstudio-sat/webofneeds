...
public abstract class BaseEventListener implements EventListener {
    ...
    public final void onEvent(final Event event) throws Exception {
        ...
        try {
            ...
            logger.warn(
                    "Caught Throwable during event processing by EventListener. Swallowing and publishing an ErrorEvent",
                    e);
    ...
    /**
     * Publishes an event indicating that the listener is finished. Useful for chaining listeners. Only use when this is
     * really the case.
     */
    ...
    /**
     * Determines whether the given event should be processed or ignored. If it is ignored, it is not counted and does
     * not influence the listener's behavior. The default implementation accepts all events.
     * 
     * @param event
     * @return
     */
