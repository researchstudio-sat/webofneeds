  ...
  protected void doRun(Event event, EventListener executingListener) throws Exception {
          ...
          protected void doRun(Event event, EventListener executingListener) throws Exception {
            ...
            if (needUriFromProducer != null) {
              ...
              if (needURI != null) {
                ...
                bus.publish(new CreateNeedCommandEvent(dataset, botContextWrapper.getFactoryListName(), usedForTesting,
                    doNotMatch));
              }
            }
          }
    ...
    bus.subscribe(CreateNeedCommandSuccessEvent.class,
        new ActionOnEventListener(ctx,
            new MultipleActions(ctx, new DecrementCounterAction(ctx, creationUnfinishedCounter), // decrease the
    ...
    bus.subscribe(CreateNeedCommandEvent.class,
        new ActionOnEventListener(ctx, new MultipleActions(ctx, new ExecuteCreateNeedCommandAction(ctx), // execute the
                                                                                                         // need
                                                                                                         // creation for
                                                                                                         // the need in
                                                                                                         // the event
            new IncrementCounterAction(ctx, needCreationStartedCounter), // increase the needCreationStartedCounter to
                                                                         // signal another pending creation
            new IncrementCounterAction(ctx, creationUnfinishedCounter) // increase the creationUnfinishedCounter to
                                                                       // signal another pending creation
        )));
