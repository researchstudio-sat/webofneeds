...
import won.bot.framework.events.listener.AbstractDoOnceAfterNEventsListener;
...
public class ActionOnceAfterNEventsListener extends AbstractDoOnceAfterNEventsListener
{
  ...
  public ActionOnceAfterNEventsListener(final EventListenerContext context, final EventFilter eventFilter, final int targetCount, final EventBotAction task)
  {
    super(context, eventFilter, targetCount);
    this.task = task;
  }

  public ActionOnceAfterNEventsListener(final EventListenerContext context, final String name, final int targetCount, final EventBotAction task)
  {
    super(context, name, targetCount);
    this.task = task;
  }

  public ActionOnceAfterNEventsListener(final EventListenerContext context, final String name, final EventFilter eventFilter, final int targetCount, final EventBotAction task)
  {
