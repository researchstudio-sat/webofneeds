...
/**
 * EventListener that counts the events it is subscribed for and after having seen a specified number of events,
 * publishes a specified event. After that, counters are reset and the listener will eventually publish another such
 * event.
 */
public class PublishEventAfterNEventsListener<T extends Event> extends BaseEventListener {
    ...
    public PublishEventAfterNEventsListener(final EventListenerContext context, final String name,
            final int targetCount, final Class<T> eventClassToPublish) {
        super(context, name);
        this.targetCount = targetCount;
        this.eventClassToPublish = eventClassToPublish;
    }
