...
export base_folder=/usr/share/webofneeds/hackathon
...
if [ "$remove_all_data" = true ] ; then
  ...
  ssh root@satvm02 rm -rf $base_folder/won-server-certshackathon
...
ssh root@satvm02 mkdir -p $base_folder/won-server-certshackathon
...
ssh root@satvm02 mkdir -p $base_folder/won-server-certshackathon
scp won_certificate_passwd_file root@satvm02:$base_folder/won-server-certshackathon/won_certificate_passwd_file
...
# copy the hackathon skin to the custom skin folder that get used by this instance
ssh root@satvm02 mkdir -p $base_folder/custom_owner_skin
scp -r $WORKSPACE/webofneeds/won-owner-webapp/src/main/webapp/static/skin/hackathon/* root@satvm02:$base_folder/custom_owner_skin/
