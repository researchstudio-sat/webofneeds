 ...
 * type is CREATE, the processor adds the appropriate public key to the need's
 ...
 * pair for all need it processes. If the <code>fixedPrivateKeyAlias</code>
 * property is not set, the processor generates one keypair per need, using the
 * need URI as the keypair's alias. This processor should be removed when
 * user's/need's key management and signing happens in the Owner Client
