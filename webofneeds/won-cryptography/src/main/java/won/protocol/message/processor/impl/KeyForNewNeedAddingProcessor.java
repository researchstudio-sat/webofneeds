 ...
 *
 * If the <code>fixedPrivateKeyAlias</code> property is set, the processor generates at most one key pair with that
 * alias and uses that key pair for all need it processes.
 *
 * If the <code>fixedPrivateKeyAlias</code> property is not set, the processor generates one keypair per need, using the
 * need URI as the keypair's alias.
 *
 * This processor should be removed when user's/need's key management and signing happens in the Owner Client (browser).
 *
