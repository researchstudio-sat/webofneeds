 ...
 * <p/>
 * A utility class for assembling the calculated signature inside the GraphCollection into the original Dataset; for
 * removing signature graphs from a Dataset.
 */
public class WonAssembler {
    ...
    /**
     * Assumes that namedSignedGraph is GraphCollection containing exactly one named graph that is part of the Dataset
     * graphOrigin, and that for this named graph the signature is already calculated inside the GraphCollection
     * internally (i.e. an Algorithm's methods canonicalize(namedSignedGraph), postCanonicalize(namedSignedGraph),
     * hash(namedSignedGraph, envHashAlgorithm), postHash(namedSignedGraph), sign(namedSignedGraph, privateKey) have
     * already been applied.
     *
     * The method assembles the signature from provided GraphCollection into the origin Dataset by putting signature
     * triples inside the default graph of the Dataset. Intended for the use by WonSigner.
     *
     * @param namedSignedGraph
     *            GraphCollection containing one named graph with its calculated signature
     * @param graphOrigin
     *            Dataset that contains the graph that has was used to construct the namedSignedGraph
     * @throws Exception
     */
    @Deprecated
     ...
     *
     * The method assembles the signature from provided GraphCollection into the origin Dataset by putting signature
     * triples inside the named graph of the Dataset. Intended for the use by WonSigner.
     *
     * @param namedSignedGraph
     *            GraphCollection containing one named graph with its calculated signature
     * @param graphOrigin
     *            Dataset that contains the graph that has was used to construct the namedSignedGraph
     * @param sigGraphURI
     *            the name (URI) of the graph that should be assigned to the signature graph
     * @throws Exception
     */
    ...
    /**
     * Removes signature graphs from the Dataset. Can be useful to use after verification is done, when the signatures
     * are no longer required for further actions on the signed data of the Dataset.
     *
     * @param dataset
     *            from which graphs representing signatures have to be removed
     */
    ...
    private static void verifyGraphCollectionContainsExactlyOneNamedGraph(GraphCollection gc) {
        ...
        if (graphs.size() == 1 && !graphs.get(0).getName().isEmpty()) {
            ...
            throw new IllegalArgumentException(
                    WonAssembler.class.getName() + " expects exactly one named graph, found " + (graphs.size() - 1));
        }
    }
