...
public class ModelConverter {
     ...
     *
     * It is required, that GraphCollection stores the prefixes (if present), but that they are not applied (i.e. its
     * applyPrefixes() method should not be called). Otherwise there could be errors when converting, since
     * Signingframework when applying prefixes just looks whether the resource uri starts with that prefix uri, which
     * would be true in both cases below:
     * 
     * @prefix : <http://www.example.com/resource/need/12#> .
     * @prefix need: <http://www.example.com/resource/need/12> . Also, applying prefixes in NamedGraph in cases like
     * @prefix : <http://www.example.com/resource/need/12/> <http://www.example.com/resource/need/12/connections/> a
     *         ldp:Container . would result in a wrong RDF triple: :connections/ a ldp:Container .
     */
     ...
     *
     * Resulting GraphCollection contains exactly one NamedGraph that corresponds to the provided Model (modelURI) from
     * the provided Dataset. The resulting GraphCollection stores the prefixes (if were present in the Datastore), but
