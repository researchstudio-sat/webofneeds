...
public class ModelConverterTest
{
  ...
  public void modelToGraphCollectionTest() throws Exception {
    ...
    //GraphCollection gc = ModelConverter.modelToGraphCollection(modelName, model, pm);
  ...
  public void namedGraphToModelTest() throws Exception {
    ...
    for (NamedGraph g : graphs) {
      if (!g.getName().isEmpty() && modelName.endsWith(g.getName().substring(1))) {
        //g.getName().equals(":OWN1")) {
        //graph = g;
        graphName = g.getName();
        break;
      }
    }
    //above replace with createTestNamedGraph(fromResource)
