...
public class WonSignerTest {
  ...
  public void testSignCreatedNeedOwnerEvent() throws Exception {
    // create dataset that contains need core data graph, its signature, and message
    // envelope generated by the owner
    Dataset testDataset = TestSigningUtils.prepareTestDatasetFromNamedGraphs(RESOURCE_FILE,
        new String[] { NEED_CORE_DATA_URI, NEED_CORE_DATA_SIG_URI, EVENT_ENV1_URI });

    // sign it
    WonSigner signer = new WonSigner(testDataset);
    signer.sign(keys.getPrivateKey(TestSigningUtils.needCertUri), TestSigningUtils.needCertUri,
        keys.getPublicKey(TestSigningUtils.needCertUri), new String[] { EVENT_ENV1_URI });
  ...
  public void testSignCreatedNeedNodeEvent() throws Exception {
    ...
    // create dataset that contains need core data graph, its signature, message
    // envelope generated by the owner,
    ...
    Dataset testDataset = TestSigningUtils.prepareTestDatasetFromNamedGraphs(RESOURCE_FILE, new String[] {
        NEED_CORE_DATA_URI, NEED_CORE_DATA_SIG_URI, EVENT_ENV1_URI, EVENT_ENV1_SIG_URI, EVENT_ENV2_URI });
