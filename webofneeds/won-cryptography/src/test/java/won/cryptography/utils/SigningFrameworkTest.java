...
public class SigningFrameworkTest {
    ...
    private static final String[] SIGNING_FW_TEST_FILES = new String[] { "/example_01.trig", "/example_02.trig",
                    "/example_03.trig",
    ...
    // read of signingframework unsigned examples by jena works? and write produces
    // orig input?
    // yes, but the triples not enclosed in a graph are written back inside unnamed
    // (default) graph
    // (this is in accordance with TriG spec) and blank node is replaced by
    // anonymous object
    // (this is also OK)
    @Test
    public void readWriteDatasetExampleTests() throws Exception {
        for (String testFile : SIGNING_FW_TEST_FILES) {
            ...
            is.close();
            // Dataset dataset =
            // RDFDataMgr.loadDataset(SigningFrameworkTest.class.getResource(SIGNING_FW_TEST_FILE).getPath());
    ...
    // read of signingframework signed examples by jena works? and write produces
    // orig input?
    // Yes, but if the graph name is not a blank node (example_05.trig was therefore
    // modified
    // accordingly), and without nested graph (i.e only one iteration of signing can
    // apply).
    @Test
    ...
    // read of webofneeds examples converted by jena to trig and read by
    // signingframework works?
}
