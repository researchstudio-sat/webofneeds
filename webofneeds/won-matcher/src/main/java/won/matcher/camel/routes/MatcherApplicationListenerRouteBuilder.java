...
public class MatcherApplicationListenerRouteBuilder extends RouteBuilder  {
    ...
    public MatcherApplicationListenerRouteBuilder(CamelContext camelContext, Set<String> endpoints,
                                                  URI remoteEndpoint) {
      super(camelContext);
    ...
    public void configure() throws Exception {
               for (int i = 0; i<endpoints.size();i++){
                   from(endpoints.get(i)).routeId("Node2MatcherRoute"+brokerUri+i)
                           ...
                           .to("bean:matcherProtocolMatcherServiceJMSBased?method=needDeactivated")
                           .otherwise()
