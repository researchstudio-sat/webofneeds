...
public class Validator {
  ...
  public static void loadOntologies() {
    ...
    sparqlPreface = "" + "PREFIX won: <" + WON_ONTOLOGY_URI + ">" + "PREFIX wonexample: <" + EXAMPLE_ONTOLOGY_URI + ">"
        ...
        + "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>" + "PREFIX gr: <http://purl.org/goodrelations/v1#>";
  }
  ...
  public void testCQHow1() {
    System.out.println("executing queries...");
    String queryString = sparqlPreface + "SELECT ?need ?needModality ?price ?currency ?lowerPrice ?upperPrice WHERE {"
    ...
    String expected1 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        ...
        + "PriceSpecification_01_1> ) " + "( ?currency = \"EUR\"^^xsd:string ) ( ?lowerPrice = \"100.0\"^^xsd:float ) "
    ...
    String expected2 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        ...
        + "PriceSpecification_01_2> ) " + "( ?currency = \"EUR\"^^xsd:string ) ( ?lowerPrice = \"130.0\"^^xsd:float ) "
  ...
  public void testCQWhat4() {
    ...
    String queryString = sparqlPreface + "SELECT ?need ?height ?heightV ?heightM ?width ?widthV ?widthM "
        ...
        + "?need won:hasContent ?content." + "?content won:hasHeight ?height." + "?height gr:hasValue ?heightV."
        + "?height gr:hasUnitOfMeasurement ?heightM." + "?content won:hasWidth ?width." + "?width gr:hasValue ?widthV."
        + "?width gr:hasUnitOfMeasurement ?widthM." + "?content won:hasDepth ?depth." + "?depth gr:hasValue ?depthV."
        + "?depth gr:hasUnitOfMeasurement ?depthM." + "?content won:hasWeight ?weight."
        + "?weight gr:hasValue ?weightV." + "?weight gr:hasUnitOfMeasurement ?weightM." + "}";
    Query query = QueryFactory.create(queryString);
    QueryExecution qExec = QueryExecutionFactory.create(query, ontModel);
    List<String> actualList = new ArrayList<String>();
    try {
    ...
    String expected = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?height = <" + EXAMPLE_ONTOLOGY_URI
        + "CouchHeight> ) ( ?heightV = \"50\"^^rdfs:Literal ) " + "( ?heightM = \"cm\"^^xsd:string ) ( ?width = <"
        + EXAMPLE_ONTOLOGY_URI + "CouchWidth> ) "
  ...
  public void testCQWhat6() {
    ...
    String queryString = sparqlPreface + "SELECT ?need ?height ?width ?depth ?weight WHERE {"
        + "?need rdf:type won:Need. " + "?need won:hasContent ?content." + "?content won:hasHeight ?height."
        + "?content won:hasWidth ?width." + "?content won:hasDepth ?depth." + "?content won:hasWeight ?weight." + "}";
    Query query = QueryFactory.create(queryString);
    QueryExecution qExec = QueryExecutionFactory.create(query, ontModel);
    List<String> actualList = new ArrayList<String>();
    try {
    ...
    String expected = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?height = <" + EXAMPLE_ONTOLOGY_URI
        ...
        + EXAMPLE_ONTOLOGY_URI + "CouchDepth> ) " + "( ?weight = <" + EXAMPLE_ONTOLOGY_URI + "CouchWeight> )";
    assertThat(actualList, hasItems(expected));
  }
  ...
  public void testCQWhen3() {
    ...
    String expected1 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_2> ) " + "( ?p = <" + WON_ONTOLOGY_URI + "recurIn> ) ( ?o = \"P7D\"^^xsd:duration )";
    String expected2 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_2> ) " + "( ?p = <" + WON_ONTOLOGY_URI
    ...
    String expected4 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_2> ) " + "( ?p = <" + WON_ONTOLOGY_URI + "recurTimes> ) ( ?o = 0 )";
    String expected5 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_1> ) " + "( ?p = <" + WON_ONTOLOGY_URI + "recurIn> ) ( ?o = \"P7D\"^^xsd:duration )";
    String expected6 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_1> ) " + "( ?p = <" + WON_ONTOLOGY_URI
    ...
    String expected8 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_1_1> ) " + "( ?p = <" + WON_ONTOLOGY_URI + "recurTimes> ) ( ?o = 0 )";
    String expected9 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_2> ) " + "( ?timeConstraint = <" + EXAMPLE_ONTOLOGY_URI
        + "TimeConstraint_01_2> ) " + "( ?p = <" + WON_ONTOLOGY_URI
  ...
  public void testCQWhere1() {
    ...
    String expected1 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?location = <" + EXAMPLE_ONTOLOGY_URI + "Thurngasse_8> ) "
        + "( ?p = <http://www.w3.org/2003/01/geo/wgs84_pos#latitude> ) " + "( ?o = \"48.218746\"^^xsd:float )";
    String expected2 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_1> ) " + "( ?location = <" + EXAMPLE_ONTOLOGY_URI + "Thurngasse_8> ) "
        + "( ?p = <http://www.w3.org/2003/01/geo/wgs84_pos#longitude> ) " + "( ?o = \"16.360283\"^^xsd:float )";
    String expected3 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?needModality = <"
        + EXAMPLE_ONTOLOGY_URI + "NeedModality_01_2> ) " + "( ?location = <" + EXAMPLE_ONTOLOGY_URI + "Österreich> ) "
  ...
  public void testCQWhere2() {
    ...
    assertEquals("wrong number of results", 2, actualList.size());
    String expected1 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?location = <" + EXAMPLE_ONTOLOGY_URI
        + "Thurngasse_8> ) ( ?concealed = true )";
    String expected2 = "( ?need = <" + EXAMPLE_ONTOLOGY_URI + "Need_01> ) " + "( ?location = <" + EXAMPLE_ONTOLOGY_URI
        + "Österreich> ) ( ?concealed = false )";
    assertThat(actualList, hasItems(expected1, expected2));
  }
