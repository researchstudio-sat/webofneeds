...
public class ConnectMessageFromOwnerProcessor extends AbstractFromOwnerCamelProcessor
{
  public void process(final Exchange exchange) throws Exception {
    ...
    if (connectionURI.isPresent()) {
      ...
      if (!con.isPresent()) throw new NoSuchConnectionException(connectionURI.get());
      ...
      if (userDefinedFacetURI.isPresent() && !userDefinedFacetURI.equals(con.get().getFacetURI())) {
          throw new IllegalStateException("Cannot process CONNECT message FROM_OWNER. Specified facet uri conflicts with existing connection data");
      }
      // remote facet uri: may be set on the connection, in which case we may have a conflict
      if (con.get().getRemoteFacetURI() != null && userDefinedRemoteFacetURI != null && !con.get().getRemoteFacetURI().equals(userDefinedRemoteFacetURI)) {
          throw new IllegalStateException("Cannot process CONNECT message FROM_OWNER. Specified remote facet uri conflicts with existing connection data");
      }
      ...
      // facets are set in the connection now.
        ...
        if (con.isPresent()) {
            ...
            con = connectionRepository.findOneByNeedURIAndRemoteNeedURIAndFacetURIAndNullRemoteFacetForUpdate(senderNeedURI, receiverNeedURI, actualFacetURI.get());
            if (con.isPresent()) {
                ...
                con = Optional.of(dataService.createConnection(connectionUri, senderNeedURI, receiverNeedURI, null, actualFacet.getFacetURI(), actualFacet.getTypeURI(), 
                                                    actualRemoteFacetURI.get(), ConnectionState.REQUEST_SENT, ConnectionEventType.OWNER_OPEN));
            }
        } 
    }
