  ...
  public void process(final Exchange exchange) throws Exception {
    ...
    if (!connectionURIFromWonMessage.isPresent()) {
      ...
      if (!con.isPresent()) {
        ...
        con = Optional.of(dataService.createConnection(connectionUri, wonMessage.getReceiverNeedURI(), wonMessage.getSenderNeedURI(),
    ...
    //now perform checks
    if (!con.isPresent()) throw new IllegalStateException("connection must not be null");
    if (con.get().getRemoteNeedURI() == null) throw new IllegalStateException("remote need uri must not be null");
    if (!con.get().getRemoteNeedURI().equals(wonMessage.getSenderNeedURI())) throw new IllegalStateException("the remote need uri of the connection must be equal to the sender need uri of the message");
