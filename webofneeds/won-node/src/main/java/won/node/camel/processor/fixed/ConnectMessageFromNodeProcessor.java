  ...
  public void process(final Exchange exchange) throws Exception {
    ...
    if (connectionURI != null) {
      ...
      if (con.getRemoteFacetURI() != null && !remoteFacetURI.equals(con.getRemoteFacetURI())) {
          throw new IllegalStateException("Cannot process CONNECT message FROM_EXTERNAL. Specified facet uris conflict with existing connection data");
      }
    } else {
      ...
      Optional<Connection> conOpt = connectionRepository.findOneByNeedURIAndRemoteNeedURIAndFacetURIAndRemoteFacetURIForUpdate(needUri, remoteNeedUri, facet.getFacetURI(), remoteFacetURI);
      if (conOpt.isPresent()) {
          con = conOpt.get();
      } else {
          //did not find such a connection. It could be that the connection exists, but without a remote facet 
          conOpt = connectionRepository.findOneByNeedURIAndRemoteNeedURIAndFacetURIAndNullRemoteFacetForUpdate(needUri, remoteNeedUri, facet.getFacetURI());
          if (conOpt.isPresent()) {
    ...
    if (con == null){
      ...
      con = dataService.createConnection(connectionUri, needUri, remoteNeedUri, remoteConnectionUri, facet.getFacetURI(), facet.getTypeURI(), remoteFacetURI,
