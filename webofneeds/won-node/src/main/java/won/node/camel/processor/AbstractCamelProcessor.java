...
public abstract class AbstractCamelProcessor implements Processor {
     ...
     * Sends a system message to the owner without facet processing. Useful for Response messages.
     *
     * Allows for adding the ownerApplicationId to the exchange used during creation and sending of the system message.
     * This is useful for cases in which the owner application cannot determined otherwise, which can happen when need
     * creation fails.
     *
     * If that value is non-null, it is set as the 'ownerApplicationId' header, which is used in
     * AbstractCamelProcessor#sendMessageToOwner(..) as a fallback to determine the recipients of the message to be
     * sent.
     *
     * @param message
