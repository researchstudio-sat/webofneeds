...
@Component @FixedMessageReactionProcessor(direction = WONMSG.TYPE_FROM_EXTERNAL_STRING, messageType = WONMSG.TYPE_CONNECT_STRING) public class ConnectMessageFromNodeReactionProcessor
  ...
  private void sendAutoOpenForConnect(WonMessage connectMessageToReactTo) {
    ...
    WonMessage msg = WonMessageBuilder.setMessagePropertiesForOpen(messageURI, connectMessageToReactTo,
        "Connection request accepted automatically by WoN node").setWonMessageDirection(WonMessageDirection.FROM_SYSTEM)
        .build();
