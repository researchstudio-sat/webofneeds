...
public class PersistingWonMessageProcessor implements WonMessageProcessor {
    ...
    @Override
    // we use READ_COMMITTED because we want to wait for an exclusive lock will accept data written by a concurrent
    // transaction that commits before we read
    ...
    /**
     * If we are saving response message, update original massage with the information about response message uri
     * 
     * @param message
     *            response message
     */
    ...
    private EventContainer loadOrCreateEventContainer(final WonMessage wonMessage, final URI parent) {
        ...
        if (WonMessageType.CREATE_NEED.equals(type)) {
            // create a need event container with null parent (because it will only be persisted at a later point in
            // time)
            ...
            // create a connection event container witn null parent (because it will only be persisted at a later point
            // in
            // time)
