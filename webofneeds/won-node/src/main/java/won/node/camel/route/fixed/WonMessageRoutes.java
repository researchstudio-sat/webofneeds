...
public class WonMessageRoutes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        onException(CannotAcquireLockException.class, JpaSystemException.class) // due to our transaction isolation, we
                ...
                .maximumRedeliveries(2).redeliveryDelay(1000).handled(true).routeId("retryAfterConcurrentUpdate");
        ...
        from("activemq:queue:OwnerProtocol.in?concurrentConsumers=5").routeId("activemq:queue:OwnerProtocol.in")
                ...
                .to("bean:wonMessageIntoCamelProcessor").to("direct:checkMessage").to("direct:addEnvelopeAndTimestamp")
                .to("bean:directionFromOwnerAdder")
                // route to msg processing logic
                .to("direct:OwnerProtocolLogic");

        /**
         * Owner protocol, outgoing. The well-formed, signed message is expected to be in the body.
         */
        from("seda:OwnerProtocolOut?concurrentConsumers=5")
                ...
                // e.g. when client's public keys changed and hence their queuenames did, too. Added the onException
                // part to deal
                // with that.
        ...
        // .to("bean:needDeleter"); // --> check if the outgoing message is a success response to a delete message so we
        // can delete the need data
        ...
        /**
         * System-to-owner messages: add timestamp, sign but do not process effects etc - just send to owner. Intended
         * for messages that do not need facet processing (like response messages).
         */
        from("seda:SystemMessageToOwner?concurrentConsumers=5").routeId("seda:SystemMessageToOwner")
                .transacted("PROPAGATION_NEVER").to("bean:wonMessageIntoCamelProcessor")
                .setHeader(WonCamelConstants.DIRECTION_HEADER,
                        new ConstantURIExpression(URI.create(WONMSG.TYPE_FROM_SYSTEM_STRING)))
                // TODO: as soon as messages are signed when they reach this route, perform signature/wellformedness
                // checks here?
        ...
        /**
         * Message processing: expects messages from OwnerProtocolIn and SystemMessageIn routes. Messages will be
         * processed, effects executed, and then sent to recipient(s)
         */
        from("direct:OwnerProtocolLogic").routeId("direct:OwnerProtocolLogic")
                ...
                .setHeader(WonCamelConstants.ORIGINAL_MESSAGE_HEADER, header(WonCamelConstants.MESSAGE_HEADER)).choice()
                .when(
                        ...
                        PredicateBuilder.and(header(WonCamelConstants.ORIGINAL_MESSAGE_HEADER).isNotNull(),
                ...
                // for that, we have to re-instate the original (not the outbound) messagge, so we reply to the right
                // one
                ...
                .log(LoggingLevel.DEBUG, "suppressing sending of message to owner because the header '"
        ...
        from("activemq:queue:NeedProtocol.in?concurrentConsumers=5").routeId("WonMessageNodeRoute").choice()
                ...
                .to("direct:checkMessage").to("direct:addEnvelopeAndTimestamp").to("bean:directionFromExternalAdder")
                .to("direct:storeAndRespond")
                // put the local connection URI into the header
                .setHeader(WonCamelConstants.CONNECTION_URI_HEADER,
                ...
                .endChoice().end();
        /**
         * Need protocol, outgoing We add our signature, but we can't persist the message in this form because its URI
         * says it lives on the recipient node. The recipient will persist it.
         */
        ...
        from("activemq:queue:MatcherProtocol.in?concurrentConsumers=5").transacted("PROPAGATION_NEVER")
                ...
                // TODO as soon as Matcher can sign his messages, perform here .to("bean:wellformednessChecker") and
                // .to("bean:signatureChecker")
    ...
    /**
     * Gets the value of a property inside the message. Only works if a WonMessage object is found in the specified
     * header.
     */
