...
public class WonMessageRoutes extends RouteBuilder {
  ...
  private void logRouteStart(Exchange exchange) {
    ...
    logger.debug("starting route {}: {} type:{}, dir:{}, resp:{}, rem: {}",
        new String[] { routeId, message.getMessageURI().toString(), message.getMessageType().toString(),
            message.getEnvelopeType().toString(), message.getIsResponseToMessageURI() == null ?
            "[not a response]" :
            message.getIsResponseToMessageURI().toString(), message.getCorrespondingRemoteMessageURI() == null ?
  ...
  @Override public void configure() throws Exception {
    onException(CannotAcquireLockException.class,
        JpaSystemException.class)  //due to our transaction isolation, we may have to retry
        .log(LoggingLevel.INFO,
            "re-delivering the current message because of an exception, wonMessage header: ${header."
                + WonCamelConstants.MESSAGE_HEADER + "}").maximumRedeliveries(2).redeliveryDelay(1000).handled(true)
    ...
    from("direct:sendFailureResponse").routeId("direct:sendFailureResponse").onException(Exception.class)
    ...
    from("direct:sendSuccessResponse").routeId("direct:sendSuccessResponse").transacted("PROPAGATION_REQUIRES_NEW")
        .to("bean:parentLocker").to("bean:successResponder");   //--> seda:systemMessageIn or seda:SystemMessageToOwner
    ...
    from("direct:addEnvelopeAndTimestamp").routeId("direct:addEnvelopeAndTimestamp")

        .to("bean:envelopeAdder").to("bean:receivedTimestampAdder");
    ...
    from("direct:storeAndRespond").routeId("direct:storeAndRespond").transacted("PROPAGATION_NEVER")
        ...
        .to("direct:executeAndStoreMessage").choice().when(PredicateBuilder
        .and(header(WonCamelConstants.MESSAGE_HEADER).isNotNull(), not(new IsResponseMessagePredicate())))
    ...
    from("direct:reference-sign-persist").errorHandler(noErrorHandler()) //let the exception bubble up
        .transacted("PROPAGATION_REQUIRES_NEW").routeId("direct:reference-sign-persist").to("bean:parentLocker")
        .to("bean:referenceAdder").to("bean:signatureAdder").to("bean:persister");

    from("direct:deleteNeedIfNecessary").errorHandler(noErrorHandler()) //let the exception bubble up
        .transacted("PROPAGATION_REQUIRES_NEW").routeId("direct:deleteNeedIfNecessary").to("bean:needDeleter");
    ...
    from("activemq:queue:OwnerProtocol.in?concurrentConsumers=5").routeId("activemq:queue:OwnerProtocol.in")
        .transacted("PROPAGATION_NEVER").setHeader(WonCamelConstants.DIRECTION_HEADER,
        new ConstantURIExpression(URI.create(WONMSG.TYPE_FROM_OWNER_STRING))).to("bean:wonMessageIntoCamelProcessor")
        .to("direct:checkMessage").to("direct:addEnvelopeAndTimestamp").to("bean:directionFromOwnerAdder")
    ...
    from("seda:OwnerProtocolOut?concurrentConsumers=5")
        ...
        .log("failure during seda:OwnerProtocolOut, ignoring. Exception message: ${exception.messsage}").handled(true)
        .stop().end().transacted("PROPAGATION_NEVER").routeId("seda:OwnerProtocolOut")
     ...
     */
    from("seda:SystemMessageIn?concurrentConsumers=5").routeId("seda:SystemMessageIn").transacted("PROPAGATION_NEVER")
        .to("bean:wonMessageIntoCamelProcessor").setHeader(WonCamelConstants.DIRECTION_HEADER,
        new ConstantURIExpression(URI.create(WONMSG.TYPE_FROM_SYSTEM_STRING))).to("bean:receivedTimestampAdder")
    ...
    from("seda:SystemMessageToOwner?concurrentConsumers=5").routeId("seda:SystemMessageToOwner")
        .transacted("PROPAGATION_NEVER").to("bean:wonMessageIntoCamelProcessor").setHeader(
        WonCamelConstants.DIRECTION_HEADER, new ConstantURIExpression(URI.create(WONMSG.TYPE_FROM_SYSTEM_STRING)))
        //TODO: as soon as messages are signed when they reach this route, perform signature/wellformedness checks here?
        .to("bean:receivedTimestampAdder").to("direct:reference-sign-persist").to("direct:deleteNeedIfNecessary")
        .to("bean:toOwnerSender");   //--> seda:OwnerProtocolOut
    ...
    from("direct:OwnerProtocolLogic").routeId("direct:OwnerProtocolLogic")
        ...
        .setHeader(WonCamelConstants.ORIGINAL_MESSAGE_HEADER, header(WonCamelConstants.MESSAGE_HEADER)).choice().when(
        ...
        PredicateBuilder
            .and(header(WonCamelConstants.ORIGINAL_MESSAGE_HEADER).isNotNull(), new ShouldEchoToOwnerPredicate()))
        ...
        //header.
        .setHeader(WonCamelConstants.MESSAGE_HEADER, header(WonCamelConstants.OUTBOUND_MESSAGE_HEADER)).to(
        "direct:sendToNode")  // --> seda:NeedProtocolOut
        .endChoice().end()
        ...
        .choice().when(PredicateBuilder
        .and(header(WonCamelConstants.DIRECTION_HEADER).isEqualTo(URI.create(WONMSG.TYPE_FROM_OWNER_STRING)),
        ...
        .end().choice().when(isNotEqualTo(header(WonCamelConstants.SUPPRESS_MESSAGE_REACTION),
        ExpressionBuilder.constantExpression(Boolean.TRUE))).to("direct:reactToMessage").otherwise()
        .log(LoggingLevel.DEBUG,
            "suppressing sending of message to owner because the header '" + WonCamelConstants.SUPPRESS_MESSAGE_TO_OWNER
                + "' is 'true'").endChoice().end();
    ...
    from("activemq:queue:NeedProtocol.in?concurrentConsumers=5").routeId("WonMessageNodeRoute").choice()
        ...
        .to("bean:wonMessageIntoCamelProcessor").setHeader(WonCamelConstants.DIRECTION_HEADER,
        new ConstantURIExpression(URI.create(WONMSG.TYPE_FROM_EXTERNAL.getURI()))).to("direct:checkMessage")
        .to("direct:addEnvelopeAndTimestamp").to("bean:directionFromExternalAdder").to("direct:storeAndRespond")
        //put the local connection URI into the header
        .setHeader(WonCamelConstants.CONNECTION_URI_HEADER,
            new GetEnvelopePropertyExpression(WonCamelConstants.MESSAGE_HEADER,
                URI.create(WONMSG.RECEIVER_PROPERTY.getURI().toString()))).to("direct:invokeFacetLogic").choice().when(
        isNotEqualTo(header(WonCamelConstants.SUPPRESS_MESSAGE_TO_OWNER),
            ExpressionBuilder.constantExpression(Boolean.TRUE))).to("direct:sendToOwner").otherwise()
        .log(LoggingLevel.DEBUG,
            "suppressing sending of message to owner because the header '" + WonCamelConstants.SUPPRESS_MESSAGE_TO_OWNER
                + "' is 'true'").endChoice().end().choice().when(
        isNotEqualTo(header(WonCamelConstants.SUPPRESS_MESSAGE_REACTION),
            ExpressionBuilder.constantExpression(Boolean.TRUE))).to("direct:reactToMessage").otherwise()
        .log(LoggingLevel.DEBUG,
            "suppressing sending of message to owner because the header '" + WonCamelConstants.SUPPRESS_MESSAGE_TO_OWNER
                + "' is 'true'").endChoice().end();
     ...
     */
    from("seda:NeedProtocolOut?concurrentConsumers=5").routeId("seda:NeedProtocolOut")

        .to("bean:signatureAdder").to("bean:needProtocolOutgoingMessagesProcessor");
    /**
    ...
    from("activemq:queue:MatcherProtocol.in?concurrentConsumers=5").transacted("PROPAGATION_NEVER")
        ...
        .to("bean:directionFromExternalAdder").to("bean:receivedTimestampAdder").to("direct:processHintAndStore").to(
        "bean:toOwnerSender")   //--> seda:OwnerProtocolOut
        .otherwise().log(LoggingLevel.INFO, "could not route message");

    from("direct:processHintAndStore").transacted("PROPAGATION_REQUIRES_NEW").routeId("direct:processHintAndStore").to(
        "bean:parentLocker")
        //call the default implementation, which may alter the message.
        ...
        .to("direct:reference-sign-persist").otherwise().log(LoggingLevel.DEBUG,
        "suppressing sending of message to owner because the header '" + WonCamelConstants.IGNORE_HINT + "' is 'true'")
        .endChoice().end();
