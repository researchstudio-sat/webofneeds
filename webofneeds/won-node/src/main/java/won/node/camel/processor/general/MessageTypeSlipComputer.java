...
/**
 * Computes a message slip for message processors that are annotated with appropriate marker annotations. The annotation
 * class to look for has to be passed to this slip in the constructor.
 *
 */
public class MessageTypeSlipComputer implements InitializingBean, ApplicationContextAware, Expression {
    ...
    private String computeMessageTypeSlip(URI messageType, URI direction)
        ...
        throw new WonMessageProcessingException(String.format(
                "unexpected combination of messageType %s " + "and direction %s encountered", messageType, direction));
    }
