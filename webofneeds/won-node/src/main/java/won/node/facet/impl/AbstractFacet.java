...
/**
 * Created with IntelliJ IDEA. User: gabriel Date: 16.09.13 Time: 17:09 To change this template use File | Settings |
 * File Templates.
 */
public abstract class AbstractFacet implements FacetLogic {
    ...
    /**
     * A string that is used to create a graph URI used for need data managed by this facet.
     * 
     * @return
     */
    ...
    /**
     *
     * This function is invoked when an owner sends an open message to a won node and usually executes registered facet
     * specific code. It is used to open a connection which is identified by the connection object con. A rdf graph can
     * be sent along with the request.
     *
     * @param con
     *            the connection object
     * @param content
     *            a rdf graph describing properties of the event. The null releative URI ('<>') inside that graph, as
     *            well as the base URI of the graph will be attached to the resource identifying the event.
     * @throws NoSuchConnectionException
     *             if connectionURI does not refer to an existing connection
     * @throws IllegalMessageForConnectionStateException
     *             if the message is not allowed in the current state of the connection
     */
    @Override
    ...
    /**
     *
     * This function is invoked when an owner sends a close message to a won node and usually executes registered facet
     * specific code. It is used to close a connection which is identified by the connection object con. A rdf graph can
     * be sent along with the request.
     *
     * @param con
     *            the connection object
    ...
    /**
     * This function is invoked when an owner sends a text message to a won node and usually executes registered facet
     * specific code. It is used to indicate the sending of a chat message with by the specified connection object con
    ...
    /**
     *
     * This function is invoked when an won node sends an open message to another won node and usually executes
     * registered facet specific code. It is used to open a connection which is identified by the connection object con.
    ...
    /**
     *
     * This function is invoked when an won node sends a close message to another won node and usually executes
     * registered facet specific code. It is used to close a connection which is identified by the connection object
     * con. A rdf graph can be sent along with the request.
     *
     * @param con
     *            the connection object
     * @param content
    ...
    /**
     * This function is invoked when a won node sends a text message to another won node and usually executes registered
     * facet specific code. It is used to indicate the sending of a chat message with by the specified connection object
     * con to the remote partner.
     *
     *
     * @param con
     *            the connection object
    ...
    /**
     * This function is invoked when a matcher sends a hint message to a won node and usually executes registered facet
     * specific code. It notifies the need of a matching otherNeed with the specified match score. Originator identifies
     * the entity making the call. Normally, originator is a matching service. A rdf graph can be sent along with the
     * request.
     *
     * @param con
     *            the connection object
     * @param score
     *            match score between 0.0 (bad) and 1.0 (good). Implementations treat lower values as 0.0 and higher
     *            values as 1.0.
     * @param originator
     *            an URI identifying the calling entity
     * @param content
     *            (optional) an optional RDF graph containing more detailed information about the hint. The null
     *            releative URI ('<>') inside that graph, as well as the base URI of the graph will be attached to the
    ...
    public void hint(final Connection con, final double score, final URI originator, final Model content,
        ...
        try {
            executorService.execute(new Runnable() {
                @Override
                public void run() {
                    // here, we don't really need to handle exceptions, as we don't want to flood matching services with
                    // error messages
                    // try {
    ...
    /**
     *
     * This function is invoked when an won node sends an connect message to another won node and usually executes
     * registered facet specific code. The connection is identified by the connection object con. A rdf graph can be
    ...
    public void connectFromNeed(final Connection con, final Model content, final WonMessage wonMessage)
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                ...
                // // TODO should we introduce a new protocol method connectionFailed (because it's not an owner deny
                // but some protocol-level error)?
                // // For now, we call the close method as if it had been called from the owner side
                // // TODO: even with this workaround, it would be good to send a content along with the close (so we
    ...
    /**
     *
     * This function is invoked when an owner sends an open message to the won node and usually executes registered
     * facet specific code. The connection is identified by the connection object con. A rdf graph can be sent along
     * with the request.
     *
     * @param con
     *            the connection object
    ...
    public void connectFromOwner(final Connection con, final Model content, WonMessage wonMessage)
        ...
        // // TODO should we introduce a new protocol method connectionFailed (because it's not an owner deny but some
        // protocol-level error)?
        // // For now, we call the close method as if it had been called from the remote side
        // // TODO: even with this workaround, it would be good to send a content along with the close (so we can
    ...
    /**
     * The facet may store data into the need dataset; this method finds the graph that the facet may write to and
     * creates a new Graph in the dataset if needed.
     * 
     * @param needURI
    ...
    /**
     * The facet may store data into the need dataset; this method removes that data from the need dataset.
     * 
     * @param needURI
     */
