...
/**
 * Created with IntelliJ IDEA. User: Danijel Date: 6.2.14. Time: 15.32 To change this template use File | Settings |
 * File Templates.
 */
    ...
    public void openFromOwner(final Connection con, final Model content, final WonMessage wonMessage)
        ...
        if (con.getRemoteConnectionURI() != null) {
            executorService.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        stateManager.setStateForNeedUri(BACCState.ACTIVE.getURI(), con.getNeedURI(),
                                con.getRemoteNeedURI(), getFacetType().getURI());
                        storeBAStateForConnection(con, BACCState.ACTIVE.getURI());
                        logger.debug("Participant state {} for Coordinator {} ", stateManager
                ...
                try {
                    ...
                    // message as MODEL
                        ...
                        NodeIterator ni = message
                                .listObjectsOfProperty(message.getProperty(WON_TX.COORDINATION_MESSAGE.getURI()));
                        if (ni.hasNext()) {
                    ...
                    if ((eventType != null)) {
                        if (eventType.isBACCParticipantEventType(eventType)) {
                            ...
                            logger.debug("New state of the Participant:" + stateManager.getStateForNeedUri(
            ...
            public void run() {
                try {
                    ...
                    stateManager.setStateForNeedUri(newState.getURI(), con.getNeedURI(), con.getRemoteNeedURI(),
                            getFacetType().getURI());
                    storeBAStateForConnection(con, newState.getURI());
                    logger.debug("New state of the Participant:" + stateManager.getStateForNeedUri(con.getNeedURI(),
                            con.getRemoteNeedURI(), getFacetType().getURI()));
