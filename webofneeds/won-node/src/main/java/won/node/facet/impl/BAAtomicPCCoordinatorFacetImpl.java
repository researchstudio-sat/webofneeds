...
public class BAAtomicPCCoordinatorFacetImpl extends AbstractBAFacet {
  ...
  @Autowired private BAStateManager stateManager;
    ...
    executorService.execute(new Runnable() {
      @Override public void run() {
        try {
          ...
          {
            logger.debug(
                "Open from Participant (coordinator:" + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() +
                    " con:" + con.getConnectionURI());

            stateManager
                .setStateForNeedUri(BAPCState.ACTIVE.getURI(), URI.create(WON_TX.PHASE_FIRST.getURI().toString()),
            ...
            logger.debug(
                "Opened from Participant = coordinator:" + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() +
                    " con:" + con.getConnectionURI() + " baState:" + stateManager
                    .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString() +
                    "phase:" + BAPCState.parsePhase(stateManager
      ...
      @Override public void run() {
        try {
          ...
          if (messageForSending != null) {
            ...
            logger.debug(
                "Coordinator sends the text message:" + eventType + " coordinator:" + con.getNeedURI() + " participant:"
          ...
          //message as MODEL
            ...
            if (ni.hasNext()) {
              ...
              logger.debug(
                  "Coordinator sends the RDF message:" + eventType.getURI() + " coordinator:" + con.getNeedURI()
                      + " participant:" + con.getRemoteNeedURI() +
                      " con:" + con.getConnectionURI() + " baState:" + stateManager
                      .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()
                      +
                      " phase:" + BAPCState.parsePhase(stateManager
            ...
            if (eventType.isBAPCCoordinatorEventType(eventType)) {
              if (isCoordinatorInFirstPhase(con) && eventType.getURI().toString()
                ...
                if (canFirstPhaseFinish(con)) {
                  ...
                  logger
                      .debug("New state = coordinator:" + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() +
                          " con:" + con.getConnectionURI() + " baState:" + stateManager
      ...
      @Override public void run() {
        try {
          logger.debug("Coordinator receives message:+" + message.toString() + "= coordinator:" + con.getNeedURI()
              ...
              " con:" + con.getConnectionURI() + " baState:" + stateManager
              .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString() +
              " phase:" + BAPCState.parsePhase(
              stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                  .toString()));
          ...
          if (isCFP && (eventType.getURI().toString().equals(WON_TX.MESSAGE_EXIT.getURI().toString()) ||
            ...
            logger.debug(
                "Coordinator receives NO vote. Is it in the first phase? " + isCFP + " " + eventType.getURI().toString()
            ...
            logger.debug("New state = coordinator:" + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() +
                " con:" + con.getConnectionURI() + " baState:" + stateManager
                .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString() +
                " phase must be SECOND:" + BAPCState.parsePhase(
                stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                    .toString()));
            ...
            for (Connection tmpCon : listOfCons) {
              ...
              if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                ...
                state = BAPCState.parseString(
                    stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI())
                        .toString());  //must be Active

                logger.debug(
                    "GLOBAL NO, actions for ACTIVE states = coordinator:" + tmpCon.getNeedURI() + " participant:"
                        + tmpCon.getRemoteNeedURI() +
                        " tmpCon:" + tmpCon.getConnectionURI() + " baState:" + stateManager
                        .getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI())
                        .toString() +
                        " phase must be FIRST:" + BAPCState.parsePhase(stateManager
                        .getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                ...
                stateManager
                    .setStateForNeedUri(state.getURI(), URI.create(WON_TX.PHASE_SECOND.getURI().toString()), ownerUri,
                        needUri, getFacetType().getURI());
                storeBAStateForConnection(tmpCon, state.getURI());

                logger.debug(
                    "GLOBAL NO, changes (CANCELING) = coordinator:" + tmpCon.getNeedURI() + " participant:" + tmpCon
                        .getRemoteNeedURI() +
                        " tmpCon:" + tmpCon.getConnectionURI() + " baState:" + stateManager
                        .getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI())
                        .toString() +
                        " phase must be SECOND:" + BAPCState.parsePhase(stateManager
                        .getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                            getFacetType().getURI()).toString()));
              ...
              //process COMPLETED state
                ...
                state = BAPCState.parseString(
                    stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI())
                        .toString());  //must be COMPLETED

                logger.debug(
                    "GLOBAL NO, COMPLETED actions = coordinator:" + tmpCon.getNeedURI() + " participant:" + tmpCon
                        .getRemoteNeedURI() +
                        " tmpCon:" + tmpCon.getConnectionURI() + " baState:" + stateManager
                        .getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI())
                        .toString() +
                ...
                logger.debug(
                    "GLOBAL NO, changes (COMPENSATE) = coordinator:" + tmpCon.getNeedURI() + " participant:" + tmpCon
                        .getRemoteNeedURI() +
                        " tmpCon:" + tmpCon.getConnectionURI() + " baState:" + stateManager
                        .getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI())
                        .toString() +
          ...
          if (resendEventType != null) {
            ...
            if (BAPCEventType.isBAPCCoordinatorEventType(resendEventType)) {
              ...
              logger.debug("New state of the Coordinator:" + stateManager
                  .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()));
              logger.debug("Coordinator state indicator: {}", BAPCState.parseString(
                      stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                          .toString()).getPhase());
  ...
  private List<Connection> getAllCoordinatorParticipantConnections(Connection con) {
    List<Connection> listOfCons = connectionRepository
        .findByNeedURIAndStateAndTypeURI(con.getNeedURI(), ConnectionState.CONNECTED,
            FacetType.BAAtomicPCCoordinatorFacet.getURI());
    return listOfCons;
  }
