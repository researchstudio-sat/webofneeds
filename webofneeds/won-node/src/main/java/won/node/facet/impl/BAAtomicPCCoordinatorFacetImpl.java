...
public class BAAtomicPCCoordinatorFacetImpl extends AbstractBAFacet {
                ...
                try {
                    ...
                    if (isCoordinatorInFirstPhase(con)) // add a new Participant (first phase in progress)
                    {
                        ...
                        stateManager.setStateForNeedUri(BAPCState.ACTIVE.getURI(),
                                URI.create(WON_TX.PHASE_FIRST.getURI().toString()), con.getNeedURI(),
                        ...
                        logger.debug(
                                "Opened from Participant = coordinator:" + con.getNeedURI() + " participant:"
                                        + con.getRemoteNeedURI() + " con:" + con.getConnectionURI() + " baState:"
                                        + stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                                getFacetType().getURI()).toString()
                                        + "phase:"
                                        + BAPCState
                                                .parsePhase(stateManager
                                                        .getStatePhaseForNeedUri(con.getNeedURI(),
                                                                con.getRemoteNeedURI(), getFacetType().getURI())
                                                        .toString()));
            ...
            public void run() {
                try {
                    ...
                    if (messageForSending != null) {
                        ...
                        logger.debug(
                                "Coordinator sends the text message:" + eventType + " coordinator:" + con.getNeedURI()
                                        + " participant:" + con.getRemoteNeedURI() + " con:" + con.getConnectionURI()
                                        + " baState:"
                                        + stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                                getFacetType().getURI()).toString()
                                        + " phase:"
                    ...
                    // message as MODEL
                        ...
                        if (ni.hasNext()) {
                            ...
                            logger.debug(
                                    "Coordinator sends the RDF message:" + eventType.getURI() + " coordinator:"
                                            + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() + " con:"
                    ...
                    if ((eventType != null)) {
                        if (eventType.isBAPCCoordinatorEventType(eventType)) {
                            if (isCoordinatorInFirstPhase(con)
                                ...
                                // check whether all connections are in the state COMPLETED (all participant voted with
                                // YES)
                                // if yes -> send message and firstPhase is ended;
                                // if no -> wait for other participants to vote
                                if (canFirstPhaseFinish(con)) {
                                    BAPCState state = BAPCState
                                            .parseString(
                                                    stateManager
                                                            .getStateForNeedUri(con.getNeedURI(),
                                                                    con.getRemoteNeedURI(), getFacetType().getURI())
                                                            .toString());
                                    ...
                                    stateManager.setStateForNeedUri(state.getURI(),
                                            URI.create(WON_TX.PHASE_SECOND.getURI().toString()), con.getNeedURI(),
                                            con.getRemoteNeedURI(), getFacetType().getURI());
                                    storeBAStateForConnection(con, state.getURI());

                                    logger.debug(
                                            "New state = coordinator:" + con.getNeedURI() + " participant:"
                                                    + con.getRemoteNeedURI() + " con:" + con.getConnectionURI()
                                                    + " baState:"
                                                    + stateManager.getStateForNeedUri(con.getNeedURI(),
                                    ...
                                    logger.debug(
                                            "Not all votes are received from Participants! First phase can not finish.");
                                }
                            } else {
                                BAPCState state = BAPCState
                                ...
                                stateManager.setStateForNeedUri(newState.getURI(), BAPCState.getPhaseURI(newPhase),
                            ...
                            logger.debug("The eventType: {} can not be triggered by Coordinator.",
    ...
    public void sendMessageFromNeed(final Connection con, final Model message, final WonMessage wonMessage)
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                try {
                    logger.debug("Coordinator receives message:+" + message.toString() + "= coordinator:"
                            + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() + " con:"
                            + con.getConnectionURI() + " baState:"
                            + stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                    getFacetType().getURI()).toString()
                            + " phase:" + BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(),
                                    con.getRemoteNeedURI(), getFacetType().getURI()).toString()));
                    ...
                    if (isCFP && (eventType.getURI().toString().equals(WON_TX.MESSAGE_EXIT.getURI().toString())
                        ...
                        logger.debug(
                                "New state = coordinator:" + con.getNeedURI() + " participant:" + con.getRemoteNeedURI()
                        ...
                        for (Connection tmpCon : listOfCons) {
                            ...
                            if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                                ...
                                state = BAPCState.parseString(stateManager
                                        .getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()); // must
                                                                                                                     // be
                                                                                                                     // Active

                                logger.debug(
                                        "GLOBAL NO, actions for ACTIVE states = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(tmpCon.getNeedURI(),
                                                        tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()
                                                + " phase must be FIRST:"
                                                + BAPCState.parsePhase(stateManager
                                                        .getStatePhaseForNeedUri(tmpCon.getNeedURI(),
                                                                tmpCon.getRemoteNeedURI(), getFacetType().getURI())
                                                        .toString()));

                                eventType = BAPCEventType
                                ...
                                stateManager.setStateForNeedUri(state.getURI(),
                                        URI.create(WON_TX.PHASE_SECOND.getURI().toString()), ownerUri, needUri,
                                        getFacetType().getURI());
                                storeBAStateForConnection(tmpCon, state.getURI());

                                logger.debug(
                                        "GLOBAL NO, changes (CANCELING) = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(tmpCon.getNeedURI(),
                                                        tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()
                            ...
                            // process COMPLETED state
                            else if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI())
                                ...
                                logger.debug(
                                        "GLOBAL NO, COMPLETED actions = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(tmpCon.getNeedURI(),
                                                        tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()
                                ...
                                logger.debug(
                                        "GLOBAL NO, changes (COMPENSATE) = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(tmpCon.getNeedURI(),
                                                        tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()
                                ...
                                Resource r = myContent
                                        .createResource(BAPCEventType.MESSAGE_COMPENSATE.getURI().toString());
                                baseResource.addProperty(WON_TX.COORDINATION_MESSAGE, r);
                                // TODO: use new system
                                // needFacingConnectionClient.sendMessage(tmpCon, myContent, wonMessage);
                                baseResource.removeAll(WON_TX.COORDINATION_MESSAGE);
                        ...
                        BAPCState newState = state.transit(eventType); // propagate the phase, it is better to have this
                                                                       // global
                        BAPCState.Phase phase = BAPCState
                                .parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(),
                                        con.getRemoteNeedURI(), getFacetType().getURI()).toString());
                        newState.setPhase(phase);

                        URI phaseURI = stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                getFacetType().getURI());
                        stateManager.setStateForNeedUri(newState.getURI(), phaseURI, con.getNeedURI(),
                                con.getRemoteNeedURI(), getFacetType().getURI());
                        storeBAStateForConnection(con, newState.getURI());
                    ...
                    if (resendEventType != null) {
                        ...
                        if (BAPCEventType.isBAPCCoordinatorEventType(resendEventType)) {
                            ...
                            URI phaseURI = stateManager.getStatePhaseForNeedUri(con.getNeedURI(),
                                    con.getRemoteNeedURI(), getFacetType().getURI());
                            stateManager.setStateForNeedUri(newState.getURI(), phaseURI, con.getNeedURI(),
                                    con.getRemoteNeedURI(), getFacetType().getURI());
                            storeBAStateForConnection(con, newState.getURI());
    ...
    private boolean isCoordinatorInFirstPhase(Connection con) {
        ...
        for (Connection c : listOfCons) {
            ...
            BAPCState.Phase phase = BAPCState.parsePhase(
                    stateManager.getStatePhaseForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString());
    ...
    private void propagateSecondPhase(Connection con) {
        ...
        for (Connection currentCon : listOfCons) {
            state = BAPCState.parseString(stateManager
                    ...
                    .toString());
            BAPCState.Phase phase = BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(currentCon.getNeedURI(),
