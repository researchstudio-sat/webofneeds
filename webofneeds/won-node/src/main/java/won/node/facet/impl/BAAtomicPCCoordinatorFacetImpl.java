...
public class BAAtomicPCCoordinatorFacetImpl extends AbstractBAFacet{
    ...
    executorService.execute(new Runnable()
    {
      ...
      public void run()
      {
        try {
            ...
            {
              logger.debug("Open from Participant (coordinator:"+con.getNeedURI()+ " participant:"+con.getRemoteNeedURI()+
            ...
            else  // second phase in progress, new participants can not be added anymore
            {
              ...
              needFacingConnectionClient.close(con, myContent, messageEvent);
            }
          } catch (Exception e) {
        ...
        try {
          ...
          //message as MODEL
            ...
            if(ni.hasNext())
            {
              ...
              logger.debug("Coordinator sends the RDF message:"+eventType.getURI()+" coordinator:"+con.getNeedURI()+ " participant:"+con.getRemoteNeedURI()+
                " con:" +  con.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()+
                " phase:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()));
            }
            else
            ...
            if(eventType.isBAPCCoordinatorEventType(eventType))
            {
              if(isCoordinatorInFirstPhase(con) && eventType.getURI().toString().equals(WON_TX.MESSAGE_CLOSE.getURI().toString()))
              {
                ...
                if(canFirstPhaseFinish(con))
                {
                  ...
                  stateManager.setStateForNeedUri(state.getURI(), URI.create(WON_TX.PHASE_SECOND.getURI().toString()), con.getNeedURI(), con.getRemoteNeedURI(),
                    getFacetType().getURI());
              ...
              else
              {
                ...
                stateManager.setStateForNeedUri(newState.getURI(), BAPCState.getPhaseURI(newPhase), con.getNeedURI(), con.getRemoteNeedURI(),
                  getFacetType().getURI());
  ...
  public void sendMessageFromNeed(final Connection con, final Model message, final Dataset messageEvent)
    ...
    executorService.execute(new Runnable() {
      ...
      public void run() {
        try {
          logger.debug("Coordinator receives message:+"+message.toString()+"= coordinator:"+con.getNeedURI()+ " participant:"+con.getRemoteNeedURI()+
            " con:" +  con.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()+
            " phase:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()));
          ...
          {
            logger.debug("Coordinator receives NO vote. Is it in the first phase? " +isCFP+" "+eventType.getURI().toString() +con.getNeedURI() +" " + con.getRemoteNeedURI());
            ...
            for(Connection tmpCon : listOfCons)
            {
                ...
                .STATE_ACTIVE.getURI().toString()))
              {
                ...
                logger.debug("GLOBAL NO, actions for ACTIVE states = coordinator:"+tmpCon.getNeedURI()+ " participant:"+tmpCon.getRemoteNeedURI()+
                  " tmpCon:" +  tmpCon.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()+
                  " phase must be FIRST:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()));
                ...
                logger.debug("GLOBAL NO, changes (CANCELING) = coordinator:"+tmpCon.getNeedURI()+ " participant:"+tmpCon.getRemoteNeedURI()+
                  " tmpCon:" +  tmpCon.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()+
                  " phase must be SECOND:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()));


                Resource r = myContent.createResource(BAPCEventType.MESSAGE_CANCEL.getURI().toString());
                baseResource.addProperty(WON_TX.COORDINATION_MESSAGE, r);
                needFacingConnectionClient.sendMessage(tmpCon, myContent, messageEvent);
                ...
                .STATE_COMPLETED.getURI().toString()))
              {
                ...
                state = BAPCState.parseString(stateManager.getStateForNeedUri(ownerUri,
                  needUri, getFacetType().getURI()).toString());  //must be COMPLETED

                logger.debug("GLOBAL NO, COMPLETED actions = coordinator:"+tmpCon.getNeedURI()+ " participant:"+tmpCon.getRemoteNeedURI()+
                  " tmpCon:" +  tmpCon.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()+
                  " phase must be FIRST:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()));
                ...
                logger.debug("GLOBAL NO, changes (COMPENSATE) = coordinator:"+tmpCon.getNeedURI()+ " participant:"+tmpCon.getRemoteNeedURI()+
                  " tmpCon:" +  tmpCon.getConnectionURI()+ " baState:"+stateManager.getStateForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()+
                  " phase must be SECOND:"+ BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(), getFacetType().getURI()).toString()));
          ...
          else
          {
            ...
            BAPCState.Phase phase = BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString());
