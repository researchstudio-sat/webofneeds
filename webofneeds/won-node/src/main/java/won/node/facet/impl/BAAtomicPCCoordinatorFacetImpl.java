...
public class BAAtomicPCCoordinatorFacetImpl extends AbstractBAFacet {
            ...
            public void run() {
                try {
                    ...
                    if (messageForSending != null) {
                        ...
                        logger.debug("Coordinator sends the text message:" + eventType + " coordinator:"
                    ...
                    // message as MODEL
                        ...
                        if (ni.hasNext()) {
                            ...
                            logger.debug("Coordinator sends the RDF message:" + eventType.getURI() + " coordinator:"
                                            + con.getNeedURI() + " participant:" + con.getRemoteNeedURI() + " con:"
                                            + con.getConnectionURI() + " baState:"
                                            + stateManager.getStateForNeedUri(con.getNeedURI(), con
                                                            .getRemoteNeedURI(), getFacetType().getURI()).toString()
                                            + " phase:"
                                            + BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(
                            ...
                            if (isCoordinatorInFirstPhase(con) && eventType.getURI().toString()
                                ...
                                if (canFirstPhaseFinish(con)) {
                                    BAPCState state = BAPCState.parseString(stateManager
                                                    .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                                                    getFacetType().getURI())
                                                    .toString());
                                    ...
                                    logger.debug("New state = coordinator:" + con.getNeedURI() + " participant:"
                                                    + con.getRemoteNeedURI() + " con:" + con.getConnectionURI()
                                                    + " baState:"
                                ...
                                if (!(isCoordinatorInFirstPhase(con) && eventType.getURI().toString()
                                    ...
                                    newPhase = BAPCState.parsePhase(stateManager
                                                    .getStatePhaseForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                                                    getFacetType().getURI())
                                                    .toString());
                                else
                ...
                try {
                    logger.debug("Coordinator receives message:+" + message.toString() + "= coordinator:"
                                    ...
                                    + stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(),
                                                    getFacetType().getURI()).toString()
                                    + " phase:"
                                    + BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(con.getNeedURI(),
                    ...
                    if (isCFP && (eventType.getURI().toString().equals(WON_TX.MESSAGE_EXIT.getURI().toString())
                        ...
                        logger.debug("Coordinator receives NO vote. Is it in the first phase? " + isCFP + " "
                                        + eventType.getURI().toString() + con.getNeedURI() + " "
                                        + con.getRemoteNeedURI());
                        state = state.transit(eventType);
                        ...
                        for (Connection tmpCon : listOfCons) {
                            ...
                            if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                                ...
                                state = BAPCState.parseString(stateManager
                                                .getStateForNeedUri(ownerUri, needUri, getFacetType().getURI())
                                                .toString()); // must be
                                                              // Active
                                logger.debug("GLOBAL NO, actions for ACTIVE states = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(
                                                                tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                                                                getFacetType().getURI()).toString()
                                                + " phase must be FIRST:"
                                                + BAPCState.parsePhase(stateManager.getStatePhaseForNeedUri(
                                                                tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                                ...
                                logger.debug("GLOBAL NO, changes (CANCELING) = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(
                                                                tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                            ...
                            // process COMPLETED state
                                ...
                                state = BAPCState.parseString(stateManager
                                                ...
                                                .toString()); // must be
                                                              // COMPLETED
                                logger.debug("GLOBAL NO, COMPLETED actions = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(
                                                                tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
                                ...
                                stateManager.setStateForNeedUri(state.getURI(),
                                                ...
                                                getFacetType().getURI());
                                storeBAStateForConnection(tmpCon, state.getURI());
                                logger.debug("GLOBAL NO, changes (COMPENSATE) = coordinator:" + tmpCon.getNeedURI()
                                                + " participant:" + tmpCon.getRemoteNeedURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForNeedUri(
                                                                tmpCon.getNeedURI(), tmpCon.getRemoteNeedURI(),
    ...
    private boolean isCoordinatorInFirstPhase(Connection con) {
        ...
        for (Connection c : listOfCons) {
            ...
            BAPCState.Phase phase = BAPCState.parsePhase(stateManager
                            .getStatePhaseForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString());
    ...
    private void propagateSecondPhase(Connection con) {
        ...
        for (Connection currentCon : listOfCons) {
            ...
            BAPCState.Phase phase = BAPCState
                            .parsePhase(stateManager
                                            .getStatePhaseForNeedUri(currentCon.getNeedURI(),
                                                            currentCon.getRemoteNeedURI(), getFacetType().getURI())
                                            .toString());
