...
/**
 * Created with IntelliJ IDEA. User: Danijel Date: 24.1.14. Time: 16.08 To change this template use File | Settings |
 * File Templates.
 */
public enum BAPCEventType {
    ...
    public static BAPCEventType getCoordinationEventTypeFromURI(final String fragment) {
        ...
        for (BAPCEventType event : BAPCEventType.values())
            if (event.name().equals("MESSAGE_"
                    + fragment.substring(fragment.lastIndexOf("#Message") + 8, fragment.length()).toUpperCase()))
                return event;
        return null;
    }
