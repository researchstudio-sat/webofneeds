          ...
          if ((eventType != null)) {
            if (eventType.isBACCCoordinatorEventType(eventType)) {
              if (isCoordinatorInFirstPhase(con)
                ...
                logger.debug("New state of the Coordinator:" + stateManager.getStateForNeedUri(con.getNeedURI(),
                    con.getRemoteNeedURI(), getFacetType().getURI()));
                logger.debug("Coordinator phase: {}", BACCState.parsePhase(stateManager
                    .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()).toString()));
  ...
  private boolean isCoordinatorInFirstPhase(Connection con) {
    ...
    for (Connection c : listOfCons) {
      ...
      if (currentStateURI == null) {
        ...
        logger.debug(
            "ignoring connection {} for phase one check as it" + " hasn't been processed by the facet logic yet",
            c.getConnectionURI());
        continue;
      }
