...
public class BAAtomicCCCoordinatorFacetImpl extends AbstractBAFacet {
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    {
                        stateManager.setStateForNeedUri(BACCState.ACTIVE.getURI(),
                            ...
                            if (isCoordinatorInFirstPhase(con)
                                ...
                                if (canFirstPhaseFinish(con)) {
                                    ...
                                    logger.debug("Current state of the Coordinator {} for participant {}: ",
                                            state.getURI().toString(), con.getRemoteNeedURI().toString());
                                ...
                                logger.debug("Current state of the Coordinator {} for participant {}: ",
                                        state.getURI().toString(), con.getRemoteNeedURI());
                                logger.debug("This Coordinator state phase: {}",
                                        BACCState.parsePhase(stateManager.getStateForNeedUri(con.getNeedURI(),
                                                con.getRemoteNeedURI(), getFacetType().getURI()).toString()));
                                ...
                                stateManager.setStateForNeedUri(newState.getURI(), BACCState.getPhaseURI(newPhase),
                                        con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI());
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (isCoordinatorInFirstPhase(con)
                            && (eventType.getURI().toString().equals(WON_TX.MESSAGE_EXIT.getURI().toString())
                        ...
                        for (Connection tmpCon : listOfCons) {
                            ...
                            if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                                    ...
                                    || stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI())
                                            .toString().equals(WON_TX.STATE_COMPLETING.getURI().toString())) {
                                ...
                                logger.debug("New state of the Coordinator must be CANCELING:"
                                        + stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()));
                        ...
                        logger.debug("New state of the Coordinator:" + stateManager.getStateForNeedUri(con.getNeedURI(),
                                con.getRemoteNeedURI(), getFacetType().getURI()));
                        logger.debug("Coordinator state phase: {}", BACCState.parsePhase(stateManager
                                .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                                .toString()));
    ...
    private boolean canFirstPhaseFinish(Connection con) {
        ...
        for (Connection tmpCon : listOfCons) {
            ownerUri = tmpCon.getNeedURI();
            needUri = tmpCon.getRemoteNeedURI();
            currentStateUri = BACCState
                    .parseString(stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString())
                    .getURI();
    ...
    private List<Connection> getAllCoordinatorParticipantConnections(Connection con) {
        List<Connection> listOfCons = connectionRepository.findByNeedURIAndStateAndTypeURI(con.getNeedURI(),
                ConnectionState.CONNECTED, FacetType.BAAtomicCCCoordinatorFacet.getURI());
        return listOfCons;
    }
    ...
    private boolean isCoordinatorInFirstPhase(Connection con) {
        ...
        logger.debug("Checking if coordinator is in the first phase for coordinator " + con.getNeedURI() + "participant"
                + con.getRemoteNeedURI() + "and connection" + con.getConnectionURI());
        ...
        for (Connection c : listOfCons) {
            ...
            if (currentStateURI == null) {
                ...
                logger.debug("ignoring connection {} for phase one check as it"
                        + " hasn't been processed by the facet logic yet", c.getConnectionURI());
                continue;
            }

            BACCState.Phase phase = BACCState.parsePhase(
