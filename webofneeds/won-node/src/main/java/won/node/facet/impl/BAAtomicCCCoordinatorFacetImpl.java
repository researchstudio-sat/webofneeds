...
public class BAAtomicCCCoordinatorFacetImpl extends AbstractBAFacet {
    ...
    executorService.execute(new Runnable() {
      @Override public void run() {
        try {
          ...
          {
            stateManager
                .setStateForNeedUri(BACCState.ACTIVE.getURI(), URI.create(WON_TX.PHASE_FIRST.getURI().toString()),
            ...
            logger.debug("*** opened from Participant = coordinator:" + con.getNeedURI() + " participant:" + con
                .getRemoteNeedURI() +
            ...
            if (eventType.isBACCCoordinatorEventType(eventType)) {
              if (isCoordinatorInFirstPhase(con) && eventType.getURI().toString()
                ...
                if (canFirstPhaseFinish(con)) {
                  ...
                  BACCState state = BACCState.parseString(
                  ...
                  logger.debug("This Coordinator state phase: {}", BACCState.parsePhase(
                      stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                          .toString()));
                ...
                stateManager.setStateForNeedUri(newState.getURI(), BACCState.getPhaseURI(newPhase), con.getNeedURI(),
                    con.getRemoteNeedURI(), getFacetType().getURI());
                storeBAStateForConnection(con, newState.getURI());

                logger.debug("New state of the Coordinator:" + stateManager
                    .getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI()));
                logger.debug("Coordinator phase: {}", BACCState.parsePhase(
                    stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                        .toString()));
          ...
          if (isCoordinatorInFirstPhase(con) && (
            ...
            for (Connection tmpCon : listOfCons) {
              ...
              if (stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                  .equals(WON_TX.STATE_ACTIVE.getURI().toString()) || stateManager
                  .getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()
                ...
                logger.debug("Current state of the Coordinator must be either ACTIVE or COMPLETING: " + state.getURI()
                    .toString());
                logger.debug("Coordinator state phase: {}", BACCState.parsePhase(
                    stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                        .toString()));
                ...
                logger.debug("Coordinator state phase must be second: {}", BACCState.parsePhase(
                    stateManager.getStateForNeedUri(con.getNeedURI(), con.getRemoteNeedURI(), getFacetType().getURI())
                        .toString()));
  ...
  private boolean canFirstPhaseFinish(Connection con) {
    ...
    for (Connection tmpCon : listOfCons) {
      ...
      currentStateUri = BACCState
          .parseString(stateManager.getStateForNeedUri(ownerUri, needUri, getFacetType().getURI()).toString()).getURI();
      if (currentStateUri.toString().equals(WON_TX.STATE_ACTIVE.getURI().toString()) || currentStateUri.toString()
          .equals(WON_TX.STATE_COMPLETING.getURI().toString()))
        return false;
    }
    return true;
  ...
  private boolean isCoordinatorInFirstPhase(Connection con) {
    ...
    for (Connection c : listOfCons) {
      ...
      logger
          .debug("checking if coordinator is in first phase for coordinator {} and participant {}", ownerUri, needUri);
      ...
      if (currentStateURI == null) {
        ...
        logger
            .debug("ignoring connection {} for phase one check as it" + " hasn't been processed by the facet logic yet",
                c.getConnectionURI());
        continue;
      }
