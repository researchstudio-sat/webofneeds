...
public class BAAtomicPCCoordinatorSocketImpl extends AbstractBASocket {
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    {
                        logger.debug("Open from Participant (coordinator:" + con.getAtomURI() + " participant:"
                                        + con.getTargetAtomURI() + " con:" + con.getConnectionURI());
                        stateManager.setStateForAtomUri(BAPCState.ACTIVE.getURI(),
                        ...
                        logger.debug("Opened from Participant = coordinator:" + con.getAtomURI() + " participant:"
                                        ...
                                        + "phase:"
                                        + BAPCState.parsePhase(stateManager.getStatePhaseForAtomUri(con.getAtomURI(),
                    ...
                    } else // second phase in progress, new participants can not be added anymore
                    {
                        ...
                        // TODO: use new system
                        // atomFacingConnectionClient.close(con, myContent, wonMessage);
                    }
                } catch (Exception e) {
                ...
                try {
                    ...
                    if (messageForSending != null) {
                        ...
                        logger.debug("Coordinator sends the text message:" + eventType + " coordinator:"
                                        + con.getAtomURI() + " participant:" + con.getTargetAtomURI() + " con:"
                    ...
                    // message as MODEL
                        ...
                        if (ni.hasNext()) {
                            ...
                            logger.debug("Coordinator sends the RDF message:" + eventType.getURI() + " coordinator:"
                                            + con.getAtomURI() + " participant:" + con.getTargetAtomURI() + " con:"
                                            + con.getConnectionURI() + " baState:"
                                            + stateManager.getStateForAtomUri(con.getAtomURI(), con
                                                            .getTargetAtomURI(), getSocketType().getURI()).toString()
                                            + " phase:"
                                            + BAPCState.parsePhase(stateManager.getStatePhaseForAtomUri(
                        ...
                        if (eventType.isBAPCCoordinatorEventType(eventType)) {
                            if (isCoordinatorInFirstPhase(con) && eventType.getURI().toString()
                                ...
                                if (canFirstPhaseFinish(con)) {
                                    ...
                                    logger.debug("New state = coordinator:" + con.getAtomURI() + " participant:"
                                                    + con.getTargetAtomURI() + " con:" + con.getConnectionURI()
                                                    + " baState:"
                                                    ...
                                                    + BAPCState.parsePhase(stateManager
                                                                    .getStatePhaseForAtomUri(con.getAtomURI(),
                                ...
                                stateManager.setStateForAtomUri(newState.getURI(), BAPCState.getPhaseURI(newPhase),
                                                con.getAtomURI(), con.getTargetAtomURI(), getSocketType().getURI());
                                storeBAStateForConnection(con, newState.getURI());
                                logger.debug("New state = coordinator:" + con.getAtomURI() + " participant:"
                                                + con.getTargetAtomURI() + " con:" + con.getConnectionURI()
                                                + " baState:" + stateManager
                                                                .getStateForAtomUri(con.getAtomURI(),
                                                                                con.getTargetAtomURI(),
                                                                                getSocketType().getURI())
                                                                .toString()
                                                + " phase:"
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    BAPCState state = BAPCState.parseString(stateManager.getStateForAtomUri(con.getAtomURI(),
                                    con.getTargetAtomURI(), getSocketType().getURI()).toString());
                    // atomicBots outcome
                    ...
                    if (isCFP && (eventType.getURI().toString().equals(WON_TX.MESSAGE_EXIT.getURI().toString())
                        ...
                        logger.debug("Coordinator receives NO vote. Is it in the first phase? " + isCFP + " "
                                        + eventType.getURI().toString() + con.getAtomURI() + " "
                                        + con.getTargetAtomURI());
                        state = state.transit(eventType);
                        ...
                        for (Connection tmpCon : listOfCons) {
                            ...
                            if (stateManager.getStateForAtomUri(ownerUri, atomUri, getSocketType().getURI()).toString()
                                ...
                                state = BAPCState.parseString(stateManager
                                                ...
                                                .toString()); // must be
                                                              // Active
                                logger.debug("GLOBAL NO, actions for ACTIVE states = coordinator:" + tmpCon.getAtomURI()
                                                + " participant:" + tmpCon.getTargetAtomURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForAtomUri(
                                                                tmpCon.getAtomURI(), tmpCon.getTargetAtomURI(),
                                                                getSocketType().getURI()).toString()
                                                + " phase must be FIRST:"
                                                + BAPCState.parsePhase(stateManager.getStatePhaseForAtomUri(
                                                                tmpCon.getAtomURI(), tmpCon.getTargetAtomURI(),
                                ...
                                logger.debug("GLOBAL NO, changes (CANCELING) = coordinator:" + tmpCon.getAtomURI()
                                                + " participant:" + tmpCon.getTargetAtomURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForAtomUri(
                                                                tmpCon.getAtomURI(), tmpCon.getTargetAtomURI(),
                            ...
                            // process COMPLETED state
                                ...
                                state = BAPCState.parseString(stateManager
                                                ...
                                                .toString()); // must be
                                                              // COMPLETED
                                logger.debug("GLOBAL NO, COMPLETED actions = coordinator:" + tmpCon.getAtomURI()
                                                + " participant:" + tmpCon.getTargetAtomURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForAtomUri(
                                                                tmpCon.getAtomURI(), tmpCon.getTargetAtomURI(),
                                ...
                                stateManager.setStateForAtomUri(state.getURI(),
                                                ...
                                                getSocketType().getURI());
                                storeBAStateForConnection(tmpCon, state.getURI());
                                logger.debug("GLOBAL NO, changes (COMPENSATE) = coordinator:" + tmpCon.getAtomURI()
                                                + " participant:" + tmpCon.getTargetAtomURI() + " tmpCon:"
                                                + tmpCon.getConnectionURI() + " baState:"
                                                + stateManager.getStateForAtomUri(
                                                                tmpCon.getAtomURI(), tmpCon.getTargetAtomURI(),
                    ...
                    if (resendEventType != null) {
                        ...
                        if (BAPCEventType.isBAPCCoordinatorEventType(resendEventType)) {
                            ...
                            logger.debug("Coordinator state indicator: {}",
                                            BAPCState.parseString(stateManager.getStateForAtomUri(con.getAtomURI(),
                                                            con.getTargetAtomURI(), getSocketType().getURI())
                                                            .toString()).getPhase());
                            // eventType -> URI Resource
    ...
    private List<Connection> getAllCoordinatorParticipantConnections(Connection con) {
        List<Connection> listOfCons = connectionRepository.findByAtomURIAndStateAndTypeURI(con.getAtomURI(),
                        ConnectionState.CONNECTED, SocketType.BAAtomicPCCoordinatorSocket.getURI());
        return listOfCons;
    }
    ...
    private boolean isCoordinatorInFirstPhase(Connection con) {
        ...
        for (Connection c : listOfCons) {
            ...
            if (currentStateURI == null) {
                ...
                // " hasn't been processed by the socket logic yet",
                // c.getConnectionURI());
                continue;
            }
    ...
    private void propagateSecondPhase(Connection con) {
        ...
        for (Connection currentCon : listOfCons) {
            state = BAPCState.parseString(stateManager.getStateForAtomUri(currentCon.getAtomURI(),
                            currentCon.getTargetAtomURI(), getSocketType().getURI()).toString());
            BAPCState.Phase phase = BAPCState
                            .parsePhase(stateManager
                                            .getStatePhaseForAtomUri(currentCon.getAtomURI(),
                                                            currentCon.getTargetAtomURI(), getSocketType().getURI())
                                            .toString());
