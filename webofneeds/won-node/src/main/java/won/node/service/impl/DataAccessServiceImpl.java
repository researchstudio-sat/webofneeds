...
public class DataAccessServiceImpl implements won.node.service.DataAccessService {
     ...
     * @param facetURI
     * @param facetTypeURI
     * @param remoteFacetURI
     *            - optional if we don't know it yet.
    ...
    public Connection createConnection(final URI connectionURI, final URI needURI, final URI otherNeedURI,
        ...
        try {
            ...
            // we assume the unique key constraint on needURI, remoteNeedURI, typeURI was violated: we have to perform
            // an
            // update, not an insert
            logger.warn(
                    "caught exception, assuming unique key constraint on needURI, remoteNeedURI, typeURI was violated"
    ...
    /**
     * Adds feedback, represented by the subgraph reachable from feedback, to the RDF description of the item identified
     * by forResource
     * 
     * @param connection
     * @param feedback
    ...
    /**
     * Calculates the ATConnectionState resulting from the message in the current connection state. Checks if the
     * specified message is allowed in the connection's state and throws an exception if not.
     *
     * @param con
     * @param msg
     * @return
