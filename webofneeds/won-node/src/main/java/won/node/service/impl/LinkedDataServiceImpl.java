 ...
 * https://dvcs.w3.org/hg/ldpwg/raw-file/default/ldp-paging.html, especially for sorting
 */
public class LinkedDataServiceImpl implements LinkedDataService {
    ...
    // TODO: used to access/create event URIs for connection model rendering. Could be removed if events knew their
    // URIs.
    ...
    public Dataset listNeedURIs() {
        ...
        for (URI needURI : uris) {
            model.add(
                    model.createStatement(needListPageResource, RDFS.member, model.createResource(needURI.toString())));
        }
    ...
    /***
     * ETag-aware method for obtaining connection data. Currently does not take into account new events, only changes to
    ...
    public Dataset listConnectionURIs(final URI needURI, boolean deep, boolean addMetadata)
        ...
        NeedInformationService.PagedResource<Dataset, URI> containerPage = toContainerPage(connectionsUri.toString(),
                new SliceImpl<URI>(uris));
        if (deep) {
            addDeepConnectionData(containerPage.getContent(), uris);
        }
        if (addMetadata) {
    ...
    @Override
    @Transactional
    public NeedInformationService.PagedResource<Dataset, URI> listConnectionURIs(final int page, final URI needURI,
        ...
        Slice<URI> slice = needInformationService.listConnectionURIs(needURI, page, preferredSize, messageType,
                timeSpot);
        URI connectionsUri = this.uriService.createConnectionsURIForNeed(needURI);
        NeedInformationService.PagedResource<Dataset, URI> containerPage = toContainerPage(connectionsUri.toString(),
                slice);
    ...
    @Override
    @Transactional
    public NeedInformationService.PagedResource<Dataset, URI> listConnectionURIsBefore(final URI needURI,
            URI beforeEventURI, final Integer preferredSize, final WonMessageType messageType, final Date timeSpot,
            boolean deep, boolean addMetadata) throws NoSuchNeedException, NoSuchConnectionException {
        Slice<URI> slice = needInformationService.listConnectionURIsBefore(needURI, beforeEventURI, preferredSize,
    ...
    @Override
    @Transactional
    public NeedInformationService.PagedResource<Dataset, URI> listConnectionURIsAfter(final URI needURI,
            URI resumeConnURI, final Integer preferredSize, final WonMessageType messageType, final Date timeSpot,
            boolean deep, boolean addMetadata) throws NoSuchNeedException, NoSuchConnectionException {
        Slice<URI> slice = needInformationService.listConnectionURIsAfter(needURI, resumeConnURI, preferredSize,
                messageType, timeSpot);
        URI connectionsUri = this.uriService.createConnectionsURIForNeed(needURI);
        NeedInformationService.PagedResource<Dataset, URI> containerPage = toContainerPage(connectionsUri.toString(),
                slice);
    ...
    private String addPageQueryString(String uri, int page) {
        // TODO: simple implementation for adding page number to uri - breaks as soon as other query strings are
        // present!
     ...
     *             https://www.w3.org/TR/2013/WD-ldp-20130730/. As of state Feb 2016
     *             (https://www.w3.org/TR/2015/REC-ldp-20150226/) the paged resource should not contain any paging
     *             information as part of resource description, this information is conveyed vie HEADERs. Therefore,
     *             this method should no longer be used.
     */
    @Deprecated
    private Resource createPage(final Model model, final String containerURI, final int pageNum,
            NeedInformationService.Page page) {
    ...
    /**
     * Adds the specified URI as the default prefix for each model in the dataset and return the dataset.
     * 
     * @param dataset
     * @return
     */
    ...
    /**
     * Returns a container resource with the messageUris of all MessageEventPlaceholder objects in the slice. If deep ==
     * true, the event datasets are added, too.
     * 
     * @param containerUri
     * @param slice
     * @param deep
