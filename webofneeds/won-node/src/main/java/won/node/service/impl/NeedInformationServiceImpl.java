...
public class NeedInformationServiceImpl implements NeedInformationService {
    ...
    public Slice<Connection> listConnectionsBefore(final URI needURI, final URI resumeConnURI,
        ...
        if (messageType == null) {
            ...
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionsBeforeByActivityDate(needURI, resume, timeSpot,
                            new PageRequest(0, pageSize, Sort.Direction.DESC, "min(msg.creationDate)"));
        } else {
            resume = messageEventRepository.findMaxActivityDateOfParentAtTime(resumeConnURI, messageType, timeSpot);
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionsBeforeByActivityDate(needURI, resume, messageType, timeSpot,
                            new PageRequest(0, pageSize, Sort.Direction.DESC, "min(msg.creationDate)"));
        }
        return slice;
    }
    ...
    public Slice<Connection> listConnectionsAfter(final URI needURI, final URI resumeConnURI,
        ...
        if (messageType == null) {
            resume = messageEventRepository.findMaxActivityDateOfParentAtTime(resumeConnURI, timeSpot);
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionsAfterByActivityDate(needURI, resume, timeSpot,
                            new PageRequest(0, pageSize, Sort.Direction.ASC, "min(msg.creationDate)"));
        } else {
            resume = messageEventRepository.findMaxActivityDateOfParentAtTime(resumeConnURI, messageType, timeSpot);
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionsAfterByActivityDate(needURI, resume, messageType, timeSpot,
                            new PageRequest(0, pageSize, Sort.Direction.ASC, "min(msg.creationDate)"));
        }
        return slice;
    }
