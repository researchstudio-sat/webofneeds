...
public class NeedInformationServiceImpl implements NeedInformationService {
    ...
    public Slice<URI> listNeedURIs(int page, Integer preferedPageSize, NeedState needState) {
        ...
        if (needState == null) {
            ...
            slice = needRepository
                    .getAllNeedURIs(new PageRequest(pageNum, pageSize, Sort.Direction.DESC, "creationDate"));
        } else {
    ...
    public Slice<URI> listNeedURIsBefore(URI needURI, Integer preferedPageSize, NeedState needState) {
        ...
        if (needState == null) {
            ...
            // use 'creationDate' to keep a constant need order over requests
            slice = needRepository.getNeedURIsBefore(referenceDate, needState,
                    new PageRequest(0, pageSize, Sort.Direction.DESC, "creationDate"));
        }
        return slice;
    }
    ...
    public Slice<URI> listConnectionURIsBefore(final URI resumeConnURI, final Integer preferredPageSize,
        ...
        // use 'min(msg.creationDate)' to keep a constant connection order over requests
        slice = connectionRepository.getConnectionURIsBeforeByActivityDate(resume, timeSpot,
    ...
    public Slice<URI> listConnectionURIsAfter(final URI resumeConnURI, final Integer preferredPageSize,
        ...
        // use 'min(msg.creationDate)' to keep a constant connection order over requests
        slice = connectionRepository.getConnectionURIsAfterByActivityDate(resume, timeSpot,
    ...
    public Slice<URI> listConnectionURIs(final URI needURI, int page, Integer preferedPageSize,
        ...
        if (messageType == null) {
            ...
            if (timeSpot == null) {
                ...
                slice = connectionRepository.getConnectionURIByActivityDate(needURI, messageType, timeSpot,
                        pageRequest);
            }
        }
        return slice;
    ...
    public Slice listConnectionURIsBefore(final URI needURI, final URI resumeConnURI, final Integer preferredPageSize,
        ...
        if (messageType == null) {
            ...
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionURIsBeforeByActivityDate(needURI, resume, timeSpot,
                    new PageRequest(0, pageSize, Sort.Direction.DESC, "min(msg.creationDate)"));
        } else {
            resume = messageEventRepository.findMaxActivityDateOfParentAtTime(resumeConnURI, messageType, timeSpot);

            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionURIsBeforeByActivityDate(needURI, resume, messageType, timeSpot,
                    new PageRequest(0, pageSize, Sort.Direction.DESC, "min(msg.creationDate)"));
        }
        return slice;
    }
    ...
    public Slice listConnectionURIsAfter(final URI needURI, final URI resumeConnURI, final Integer preferredPageSize,
        ...
        if (messageType == null) {
            ...
            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionURIsAfterByActivityDate(needURI, resume, timeSpot,
                    new PageRequest(0, pageSize, Sort.Direction.ASC, "min(msg.creationDate)"));
        } else {
            resume = messageEventRepository.findMaxActivityDateOfParentAtTime(resumeConnURI, messageType, timeSpot);

            // use 'min(msg.creationDate)' to keep a constant connection order over requests
            slice = connectionRepository.getConnectionURIsAfterByActivityDate(needURI, resume, messageType, timeSpot,
                    new PageRequest(0, pageSize, Sort.Direction.ASC, "min(msg.creationDate)"));
        }
        return slice;
    }
    ...
    public Slice<URI> listConnectionEventURIsAfter(URI connectionUri, URI msgURI, Integer preferredPageSize,
        ...
        Slice<URI> slice = null;
        if (msgType == null) {
            slice = messageEventRepository.getMessageURIsByParentURIAfter(connectionUri, referenceDate,
                    new PageRequest(0, pageSize, Sort.Direction.ASC, "creationDate"));
        } else {
            slice = messageEventRepository.getMessageURIsByParentURIAfter(connectionUri, referenceDate, msgType,
                    new PageRequest(0, pageSize, Sort.Direction.ASC, "creationDate"));
        }
        return slice;
    }
    ...
    public Slice<URI> listConnectionEventURIsBefore(final URI connectionUri, final URI msgURI,
        ...
        Slice<URI> slice = null;
        if (msgType == null) {
            slice = messageEventRepository.getMessageURIsByParentURIBefore(connectionUri, referenceDate,
                    new PageRequest(0, pageSize, Sort.Direction.DESC, "creationDate"));
        } else {
            slice = messageEventRepository.getMessageURIsByParentURIBefore(connectionUri, referenceDate, msgType,
                    new PageRequest(0, pageSize, Sort.Direction.DESC, "creationDate"));
        }
        return slice;
    }
