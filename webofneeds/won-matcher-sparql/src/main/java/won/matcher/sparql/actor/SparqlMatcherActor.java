...
public class SparqlMatcherActor extends UntypedActor {
    ...
    /**
     * Produces hints for the need and possibly also 'inverse' hints. Inverse hints are hints sent to the needs we find
     * as matches for the original need.
     * 
     * The score is calculated as a function of scores provided by the embedded sparql queries: the range of those
     * scores is projected on a range of 0-1. For inverse matches, the score is always 100%, because there is only one
     * possible match - the original need. Note: this could be improved by remembering reported match scores in the
     * matcher and using historic scores for normalization, but that's a lot more work.
     */
    protected void processActiveNeedEvent(NeedEvent needEvent) throws IOException {
        ...
        Collection<HintEvent> hintEvents = produceHints(need,
                matches.stream().filter(foundNeed -> foundNeed.need.getNeedState() == NeedState.ACTIVE) // we may not
                                                                                                        // have updated
                                                                                                        // our need
                                                                                                        // state in the
                                                                                                        // database.
        ...
        if (!noHintForCounterpart) {
            ...
            List<HintEvent> inverseHintEvents = matches.stream().filter(n -> n.need.getNeedState() == NeedState.ACTIVE)
                    ...
                    .map(matchedNeed -> {
                        // query for the matched need - but only in the dataset containing the original need. If we have
                        // a match, it means
                        // that the matched need should also get a hint, otherwise it should not.
                        if (log.isDebugEnabled()) {
                            log.debug(
                                    "checking if match {} of {} should get a hint by inverse matching it in need's dataset: \n{}",
                                    new Object[] { matchedNeed.need.getNeedUri(), need.getNeedUri(),
                                            RdfUtils.toString(needDataset) });
                        }
                        List<ScoredNeed> matchesForMatchedNeed = queryNeed(matchedNeed.need,
                                Optional.of(new NeedModelWrapperAndDataset(need, needDataset)));
                        if (log.isDebugEnabled()) {
                            log.debug("match {} of {} is also getting a hint: {}",
                                    new Object[] { matchedNeed.need.getNeedUri(), need.getNeedUri(),
                                            matchesForMatchedNeed.size() > 0 });
                        }
                        return new AbstractMap.SimpleEntry<>(matchedNeed.need,
                                matchesForMatchedNeed.stream().filter(n -> n.need.getNeedState() == NeedState.ACTIVE)
                                        .filter(inverseMatch -> postFilter(matchedNeed.need, inverseMatch.need))
        ...
        return matches.stream().sorted((hint1, hint2) -> (int) Math.signum(hint2.score - hint1.score)) // sort
                ...
                .limit(config.getLimitResults()).map(hint -> {
                    ...
                    return new HintEvent(need.getWonNodeUri(), need.getNeedUri(), hint.need.getWonNodeUri(),
                            hint.need.getNeedUri(), config.getMatcherUri(), score);
                }).collect(Collectors.toList());
    }
     ...
     * 
     * @param hintEvents
     *            the collection of HintEvent to publish
     * @param needURI
     *            used for logging
     * @param inverse
     *            used for logging
    ...
    /**
     * Query for matches to the need, optionally the needToCheck is used to search in. If needToCheck is passed, it is
     * used as the result data iff the needToCheck is a match for need. This saves us a linked data lookup for data we
     * already have.
     * 
     * @param need
     * @param needToCheck
    ...
    private List<ScoredNeed> queryNeed(NeedModelWrapper need, Optional<NeedModelWrapperAndDataset> needToCheck) {
        ...
        List<ScoredNeed> needs = query.map(q -> {
            ...
            return Stream.concat(executeQuery(noHintForCounterpartQuery, needToCheck),
                    executeQuery(hintForCounterpartQuery, needToCheck)).collect(Collectors.toList());
            ...
            try {
                ...
                if ((needToCheck.isPresent())) {
                    NeedModelWrapper result = needToCheck.get().needModelWrapper.getNeedUri().equals(foundNeedUri.uri)
                            ? needToCheck.get().needModelWrapper : null;
                    if (result == null) {
                        return null;
                    }
                ...
                log.info("caught exception trying to load need URI {} : {} (more on loglevel 'debug')", foundNeedUri,
                        e.getMessage());
                if (log.isDebugEnabled()) {
                    e.printStackTrace();
                }
