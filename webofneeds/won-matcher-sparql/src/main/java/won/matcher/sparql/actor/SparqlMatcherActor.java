...
public class SparqlMatcherActor extends UntypedActor {
     ...
     * are hints sent to the needs we find as matches for the original need. The
    ...
    protected void processActiveNeedEvent(NeedEvent needEvent) throws IOException {
        ...
        Collection<HintEvent> hintEvents = produceHints(need,
                        matches.stream().filter(foundNeed -> foundNeed.need.getNeedState() == NeedState.ACTIVE) // we
                                                                                                                // may
                                                                                                                // not
                                                                                                                // have
                                                                                                                // updated
                                                                                                                ...
                                                                                                                // database.
                                                                                                                // re-check!
                                        .filter(foundNeed -> postFilter(need, foundNeed.need))
