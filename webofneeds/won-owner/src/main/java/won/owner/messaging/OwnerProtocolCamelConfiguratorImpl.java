...
public class OwnerProtocolCamelConfiguratorImpl implements OwnerProtocolCamelConfigurator {
    ...
    /**
     * Scheme of the remote endpoint for which camel component has already bean configured, should correspond to the
     * scheme of the endpoints for which listeners are being added. In this case, our component name can contain part
     * specific to a particular remote broker, so that they can connect to different brokers without overriding each
     * other.
     * 
     * @param endpoints
    ...
    public synchronized void addRouteForWoNNode(final URI wonNodeURI) throws CamelConfigurationFailedException {
        ...
        if (camelContext.getComponent(tempStartingComponentName) == null
            ...
            // OwnerProtocolDynamicRoutes ownerProtocolRouteBuilder = new OwnerProtocolDynamicRoutes(camelContext,
            // tempStartingComponentName);
