...
public class OwnerWonMessageSenderJMSBased implements ApplicationListener<WonNodeRegistrationEvent>, WonMessageSender {
    ...
    /**
     * The owner application calls the register() method node upon initalization (and during fixed time intervals) to
     * connect to the default won node.
     *
                ...
                new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            logger.warn(
                                    "Could not register with default won node {}. Try again later ... (reason is logged at "
                                            + "loglevel 'DEBUG')",
                                    defaultNodeURI);
