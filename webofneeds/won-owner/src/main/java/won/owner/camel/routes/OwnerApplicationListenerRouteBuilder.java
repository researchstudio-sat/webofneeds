...
public class OwnerApplicationListenerRouteBuilder extends RouteBuilder {
  ...
  @Override public void configure() throws Exception {
    ...
    from(endpoint).routeId("Node2OwnerRoute" + brokerUri).to("bean:wonMessageIntoCamelProcessor")
        .to("bean:wellformednessChecker").to("bean:uriNodePathChecker").choice().when(
        header(WonCamelConstants.MESSAGE_TYPE_HEADER).isEqualTo(URI.create(WONMSG.TYPE_HINT_STRING)))
