...
public class OwnerProtocolNeedServiceClient implements OwnerProtocolNeedServiceClientSide
{
  ...
  public void open(URI connectionURI, Model content) throws Exception {
    if (logger.isDebugEnabled()) {
      logger.debug("owner to need: OPEN called for connection {} with content {}", connectionURI, StringUtils
        .abbreviate(RdfUtils.toString(content), 200));
    }
  ...
  public void close(final URI connectionURI, Model content) throws Exception {
    if (logger.isDebugEnabled()) {
      logger.debug("owner to need: CLOSE called for connection {} with model {}", connectionURI, StringUtils
        .abbreviate(RdfUtils.toString(content), 200));
    }
    List<Connection> cons = connectionRepository.findByConnectionURI(connectionURI);
    if (cons.size() != 1)
  ...
  public ListenableFuture<URI> connect(final URI needURI, final URI otherNeedURI, final Model content)
    throws Exception {
    if (logger.isDebugEnabled()) {
      logger.debug("owner to need: CONNECT called for need {}, other need {} and content {}", new Object[]{needURI,
                                                                                                           otherNeedURI,
                                                                                                           StringUtils
                                                                                                             .abbreviate(RdfUtils
                                                                                                               .toString(content), 200)});
    }
