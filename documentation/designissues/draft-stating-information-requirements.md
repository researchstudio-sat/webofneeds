...
1. To specify matching constraints.  This is the case if 
    1. The shape is attached to a `match:seeks` node of the atom
...
1. To specify properties of interest after matching. This is the case if  
    1. The shape is embedded in the content of a message via ´[atomuri]/match:seeks´ 
...
* shapes graph: Compute the union of all graphs in the atoms `match:seeks`branches that are referenced by `[Atom] won:shapesGraph [Graph]`
...
* shapes graph: Compute the union of all graphs in the counterparts `match:seeks`branches that are referenced by `[Counterpart] won:shapesGraph [Graph]`
